...
| `Row_Split.lean` | The cut-agnostic division: on any row, under any decidable cut, the value is exactly its Real part and its Unicorn part; fusion stands at the open part under the declared standing rule; a certificate, or one witness on an infinitude row, proves the Real part; the Unicorn part as a counterexample set; the gap-to-interval bridge for the Legendre row; the exact tuples of the eight arithmetic height-cut rows with small-height Real parts computed in the kernel; the census of the twenty-three assignments (9 / 11 / 2 / 1). The kernel file of *The Cut-Agnostic Division Theorem* v2.0.0. | 27 sets, all reported as depending on no axiom |
