...
# TRISDUCTION PUBLICATION AUTOMATION · The Unified Governance Layer · v2.0.2
...
|---|---|---|---|---|
| git | `CODEX_GH_TOKEN` | `api.github.com`, `github.com` | one public repo, Contents read/write | low, public repo, revertible |
...
**Git · `1000sapients/Trisduction`, branch `main`, public.** Revision history and canonical truth. The only surface with no permanent addresses, which is what makes it safe to be wrong on and therefore the right place for truth to live.
...
**One literal survives in the tooling and it is a live drift risk.** `ia_publish.py` falls back to a hardcoded `Mohammad F Islam` when a plan carries no `creator`. A plan must therefore carry `creator` explicitly every time, and a plan without one is a HALT and never a default. See §14.1.
...
**The audio branch currently stops at preflight.** No shipped tool publishes an audio item; see the HALT at §14.2. The specification below is complete and correct and its publisher does not exist yet.
...
# 2 · PLAN (read, no credentials). --dir is REQUIRED and is a folder, not a file.
...
# 3 · AUDIO ONLY: preflight the MP3 and normalise ID3 locally.
#     STOP THERE. See the HALT at §14: no shipped tool publishes an audio item.
...
# 5 · PUBLISH on yes. THIS COMMAND IS THE WRITE. There is no --apply, no
#     --dry-run, and no confirmation inside the tool. It takes the plan path
#     as a bare positional argument and executes it immediately.
python3 tools/ia_publish.py ./plan.json

# 6 · VERIFY to LANDED, then patch the per-file description. Not before.
# 7 · DELETE the superseded file, then run the derivative census until clean.
```
...
**Read-only, no gate:** `resolve_current.py`, `verify_remote.py`, `pre_push_check.py`, `ia_plan.py`, `ia_reconcile.py`, `zenodo_reconcile.py`, `zenodo_preflight.py`. **Read-only with one opt-in local write:** `ia_audio_preflight.py`, whose `--write-tags` writes ID3 to the local file only, never to the archive, and changes the local md5 so the plan is re-run after tagging. **Write-capable:** `ia_publish.py`, `zenodo_publish.py`, `zenodo_fixup.py`.

### 14.1 · THREE HAZARDS IN THE TOOL SET, FOUND BY AUDIT AND NOT BY A RUN
...
**HAZARD 1 · The gate flags are not symmetric, and the asymmetry runs the dangerous way.** `zenodo_fixup.py` defaults to a **dry run** and needs `--apply` to write. `zenodo_publish.py` defaults to `--env sandbox` and writes nothing without `--stage` or `--mint`. **`ia_publish.py` has no argparse at all.** It takes the plan path as `sys.argv[1]` and executes it immediately: no dry run, no `--apply`, no confirmation inside the tool. A session carrying the `--apply` habit across from the Zenodo tools will fire an irreversible archive write, including an identifier claim, believing it staged something. **For IA, L1's gate is entirely procedural: the yes happens in the conversation and the command is the act.** Never invoke `ia_publish.py` to "see what it would do."
...
**HAZARD 3 · `ia_publish.py` still carries a byline literal.** Its header builder reads `m.get("creator") or "Mohammad F Islam"`. If a plan omits `creator`, the literal wins silently, which is exactly the drift mechanism §3.3 describes and forbids. **The plan must carry `creator` explicitly on every run**, and a plan without one is a HALT rather than a default. This is the last of the literals the source skill removed and it was missed.

### 14.2 · HALT · THE AUDIO BRANCH HAS NO PUBLISHER

§5.4 specifies the audio branch in full and `ia_audio_preflight.py` prepares a file for it. **No shipped tool can publish it.** `ia_publish.py` hardcodes `x-archive-meta-mediatype: texts` and `x-archive-meta-collection: opensource` in its header builder, with no branch and no override.

This matters more than an ordinary gap because §2.2 prices it: `mediatype` and `collection` are set on the item-creating call and are **staff-only afterwards**. An MP3 pushed through the current publisher becomes a `texts` item **permanently**, forfeiting the player, the derivatives, and the audio discovery surface, which is precisely the failure §5.4 exists to prevent. The tool would do it silently and report success.

**Until an audio publisher exists, the audio branch stops at preflight.** Preflight, normalise ID3, and hold. Do not hand an MP3 plan to `ia_publish.py` under any framing. The gap is carried at §17 and it is the one place in this file where a specification is ahead of its tooling in a direction that can cause permanent damage rather than merely no result.
...
Derives the identifier and the work key, probes the metadata endpoint three-valued, and prints the plan. It reads only the identifier it derives, so a prior living at a legacy identifier is invisible to it, and it reports nothing about derivatives.
...
### `tools/ia_publish.py` · write-gated, credentials by environment reference

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Executes a confirmed plan from ia_plan.py --json. Keys from IA_ACCESS_KEY /
IA_SECRET_KEY only, never printed, scrubbed from every error path."""
...
scrub = lambda s: s.replace(SEC, "[REDACTED]").replace(ACC, "[REDACTED]")
AUTH = {"authorization": "LOW %s:%s" % (ACC, SEC)}
LICENSE = "https://creativecommons.org/licenses/by/4.0/"

def enc(v):
    return "uri(%s)" % urllib.parse.quote(v, safe="") if any(ord(c) > 127 for c in v) else v

def meta_headers(m, desc):
    h = {"x-amz-auto-make-bucket": "1",
         "x-archive-meta-mediatype": "texts",
         "x-archive-meta-collection": "opensource",
         "x-archive-meta-language": "eng",
         "x-archive-meta-licenseurl": LICENSE,
         "x-archive-meta-creator": enc(m.get("creator") or "Mohammad F Islam"),
         "x-archive-meta-title": enc("%s · %s · 1000 Sapients"
                                     % (m["serial"], m["person"])),
...
def patch_meta(ident, target, field, value):
    ...
    try:
        urllib.request.urlopen(urllib.request.Request(
            ...
            data=body, method="POST"), timeout=120).read()
        return True, "ok"
    except Exception as e:
...
def verify(ident, name, want, fresh=False):
    ...
    u = "https://archive.org/metadata/%s" % urllib.parse.quote(ident)
...
def main():
    ...
    for op in plan:
        ...
        long_desc = None
        if fresh:
            mh = meta_headers(op["meta"], op["item_desc"])
            if sum(len(("%s: %s" % kv).encode()) for kv in mh.items()) > 6000:
                long_desc = op["item_desc"]
                mh = meta_headers(op["meta"], op["item_desc"][:400].rsplit(" ", 1)[0])
...
Probes the MP3 with ffprobe, reads and proposes the ID3 set, derives the identifier and filename under the chosen scheme, and halts on the technical floor. Requires `ffprobe` and `mutagen`.

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only audio pre-flight. Probes the file, reads and proposes ID3, derives the
identifier, and halts on anything that would be expensive to correct after upload.
Writes nothing to the archive and nothing to the file unless --write-tags is given."""
...
**The audio publisher.** §5.4 is specified and unbuildable with what ships. Writing it is a small extension of `ia_publish.py`: branch the two hardcoded headers on a `mediatype` field carried in the plan. It has to be rehearsed before it is trusted, and there is no sandbox archive to rehearse on, which is the whole difficulty. Not yet built.
...
BASELINE, measured at this edition and never carried forward:
  lines 2975 · bytes 199742 · headings 117 · description 1015/1024
