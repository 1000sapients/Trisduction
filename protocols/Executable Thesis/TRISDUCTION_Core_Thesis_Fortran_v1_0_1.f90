...
contains
  ...
  subroutine kernel_determine(M, verdict, lam, detR, kapR, idim, why, margin)
    ...
    if (.not. bok) then
       verdict = BROKEN
       why = 'orthogonalisation residual below the rounding floor; no independent frame'
       return
    end if
    call lambda_of(Q, B, lq, ld, co);  lam = lq
    kapR = cond_sym3(R)
    if (kapR >= kgate) then
...
program thesis_rows
  ...
  call prepare_rows(M, Q, ok);  call span_basis(Q, B, ok)
