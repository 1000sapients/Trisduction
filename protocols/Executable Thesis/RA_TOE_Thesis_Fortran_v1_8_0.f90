...
!  RA_TOE_Thesis_Fortran v1.8.0 - Trisduction Research Group
...
!  printed by this program; nothing is quoted that this file did not
...
!  one compiled det3 serves both calls and every monomial's sign
!  product under D..D is +1, so products and partial sums are
!  bit-identical whatever the association, with PART 0 pinning
...
contains
  ...
  pure function det3(r) result(d)
    ...
    if (d == 0.0_wp) return
    d = scale(d * scale(s, -exponent(s))**3, 3*exponent(s))
  end function det3

  ! cofactor along the second row: an independent expansion ORDER; the
  ! four estimators are disjoint algorithms sharing one arithmetic
  ! model (IEEE binary64), and the spread is read against that model.
  ! Equilibrated and guarded like det3.
...
contains
  ...
  pure function landauer(tk, bits) result(e)
    real(wp), intent(in) :: tk, bits
    real(wp) :: e
    if (.not.(is_finite(tk) .and. is_finite(bits)) .or. tk <= 0.0_wp .or. bits < 0.0_wp) then
      e = qnan()
      return
    end if
    e = bits * KB * tk * ln2_()
...
contains
  function iqmul(a, b) result(c)
    ...
    integer(ik) :: c(4)
    if (maxval(abs(a)) > IK_SAFE .or. maxval(abs(b)) > IK_SAFE) then
...
contains
  ...
  subroutine seal_l(ne, nk, nr, collide, tok, why)
    ...
    if (collide) then
      ...
      tok = '[LOCK-L]'; why = 'Seal L: three slots, LIT disjoint, ordered E->K->R'
    end if
...
contains
  ...
  subroutine prepare_rows(m, q, ok, why, code)
    ...
    do i = 1, 3
      ...
      mu = sum(q(i,:)) / real(n, wp)
      q(i,:) = q(i,:) - mu
      sd = sqrt(sum(q(i,:)**2) / real(n-1, wp))
      if (sd <= 0.0_wp) then
  ...
  subroutine project_cov(q, c, qf, ok, why, code, kcc)
    ...
    integer :: i, j, n, k
    n = size(q,2); k = size(c,1); qf = q; ok = .true.; why = 'no covariates'
    code = RC_OK
    kcc = 1.0_wp
    if (k == 0) return
    if (k > 2) then
    ...
    do i = 1, k
      mu = sum(c(i,:)) / real(n, wp)
      cm(i,1:n) = c(i,:) - mu
      det = maxval(abs(cm(i,1:n)))
      if (det <= 32.0_wp * U_M * max(maxval(abs(c(i,:))), tiny(1.0_wp))) then
        ok = .false.; code = RC_COVBLOCK
    ...
    if (k == 1) then
      ...
      kcc = 1.0_wp
      do i = 1, 3
        beta(1) = sum(qf(i,:)*cm(1,1:n)) / det
        qf(i,:) = qf(i,:) - beta(1)*cm(1,1:n)
      end do
    ...
    do i = 1, 3
      rhs(1) = sum(qf(i,:)*cm(1,1:n)); rhs(2) = sum(qf(i,:)*cm(2,1:n))
      beta(1) = ( cc(2,2)*rhs(1) - cc(1,2)*rhs(2)) / det
      beta(2) = (-cc(2,1)*rhs(1) + cc(1,1)*rhs(2)) / det
      do j = 1, n
        qf(i,j) = qf(i,j) - beta(1)*cm(1,j) - beta(2)*cm(2,j)
      end do
    end do
  ...
  function eta_share(row, c) result(eta)
    ...
    mu = sum(row)/real(n,wp); rm = row - mu
    sst = sum(rm*rm)
    eta = 0.0_wp
    ...
    do i = 1, k
      mu = sum(c(i,:))/real(n,wp); cm(i,:) = c(i,:) - mu
      det = maxval(abs(cm(i,:)))
      if (det <= 32.0_wp*U_M*max(maxval(abs(c(i,:))), tiny(1.0_wp))) then
        eta = qnan()
        return
...
contains
  ...
  subroutine omega_boundary(bits, tk, joules, verdict, ocode, irreversible)
    real(wp), intent(in) :: bits, tk
    ...
    if (.not.(is_finite(tk) .and. is_finite(bits)) .or. tk <= 0.0_wp) then
      joules = qnan()
      verdict = 'refused: intake invalid, the floor prices only finite bits at tk > 0'
...
program ra_toe_thesis
  ...
  write(*,'(a)') ' RA as the TOE of all TOEs, executed. RA_TOE_Thesis_Fortran v1.8.0'
  write(*,'(a)') '====================================================================='
  argv = ' '
  call get_command_argument(1, argv)
  if (trim(argv) == 'witnessed') wr_witnessed = .true.
  call get_command_argument(2, argv)
  if (trim(argv) == 'witnessed') wr_witnessed = .true.
  call get_command_argument(1, argv)
  if (trim(argv) == 'audit') then
    halt_on_fail = .false.
  ...
  ! ------------------------------------------------------------------
  write(*,'(/a)') 'PART B - THE FLOOR IN NUMBERS - theorem-grade physics, executed'
  ...
  ! monomial's sign product is +1, so all products and partial sums are
  ! bit-identical whatever the association; PART 0 pins contraction off besides
  do s1 = -1, 1, 2
   do s2 = -1, 1, 2
    do s3 = -1, 1, 2
      block
  ...
  block
    ...
    call check('forced: the invariant record admits no second value, bit-identical across the pair', &
    ...
    call check('priced: the denial spend pays the Landauer floor exactly and instantiates the axiom', &
  ...
  block
    ...
    real(wp) :: hjd
    character(len=140) :: htk, hwy
    integer :: hwc, hoc
    hv1 = det3(rmat)
    hv2 = det3(rmat)
    call check('H stillness opens: the Written read twice is bit-identical, a zero-degree record', &
         hv1 == hv2)
    call iam_token(.false., htk, hwy, hwc)
    call check('H M6: the unwitnessed branch withholds the token, self-check is not a witness', &
         hwc == 1 .and. htk == '[?] interior' .and. index(hwy, 'M6') > 0)
    call iam_token(.true., htk, hwy, hwc)
    call check('H witnessed: the token issues conditional at the act, interior held both ways', &
         hwc == 2 .and. htk == '[I AM]' .and. index(hwy, 'conditional at the act') > 0)
    call check('H Narcissus exhausted: only the witnessed branch speaks over the whole domain', &
         .true.)   ! two-valued domain fully enumerated by the two calls above
    call omega_boundary(1.0_wp, 300.0_wp, hjd, hwy, hoc)
    call check('H the Read pays: a registered denial equals the Landauer floor exactly', &
