...
Harvested 2026-09-16, v1.0.1, sealed under audit cycle locus at round 4 · M. F. Islam, Claude (scribe) · core Lean 4 v4.19.0 · no Mathlib · no sorry · no custom axiom.
Self-contained: carries the sealed document v5.0.1, the strip fork, and the completion test, then the two harvests.

HARVEST 1. The GOL at the RH locus: magnitude the GF(2) determinant of three unit rows named geometry, formal,
kinetic, computed in the file; the rows are chosen, and the theorem is that they are independent and that a
kinetic row copying the formal row is not; direction the witness bit through the Tongue. With the witness held the gate returns
GOL-OK, without it open; a witness restating the formal row is no axis, determinant zero. The imprint is the seal on
the witness, a residence without it, the ghost if an off-line zero were also supplied; the imprint's eight inputs
are supplied, the absence of a supplied off-line zero among them, and the theorem is what the gate returns on
them. The compartment function here is the four-input restriction of the executable thesis's cascade, agreeing
with it on these inputs; III, world-rowed, and never I.
