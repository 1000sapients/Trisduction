...
<!--
GEOMETRIC ENCYCLOPAEDIA OF THE QURʾĀN
Seed corpus. Version 1.0.1.2

Version scheme: the final component is the entry census. v1.0.1.2 carries two
entries; v2 is reached at one thousand. Entries are appended in filing order and
this file is the single growing source of the work.
...
Authored under skill geometric-encyclopedia-quran. Rendering delegates to
trisduction-publication-format Module III·E at v1.5.2 or later.

Spines: Abrogation reads from CN-PSP-ABROGATION-RELOCK-01 at SC-089 and Taḥrīf
from CN-PSP-TAHRIF-GRANT-01 at SC-088, both in the theological register. A
theological coordinate and an encyclopedia entry are one object read at two
registers: the coordinate is the settled residue with the road cut, the entry is
that residue opened out with its literature, its root and its scope guard.

Version scheme, stated so it is not ambiguous later: major.minor.patch.census.
The final component is the entry count and moves only when an entry is added; a
correction that adds no entry moves the patch component and leaves the census
standing.

Census: 2 entries, 2 major, 0 standard, 0 stubs.
-->
...
The earliest layer is tentative. Nickel's study of Muqātil b. Sulaymān (d. 150 ⁄ 767), al-Ṭabarī and the *Sīra* of Ibn Isḥāq finds that the formative-period exegesis did not read the tampering verses as textual falsification, treating the earlier scriptures as largely authentic while their holders tampered with pronunciation and interpretation, and that accusations approaching textual corruption remain isolated and tentative. Al-Qāsim b. Ibrāhīm (d. 246 ⁄ 860) does not hold textual corruption.
...
The field asks whether taḥrīf is textual or interpretive, and when the doctrine hardened. Both questions are now well answered. What it does not ask is **what kind of faculty taḥrīf is an exercise of**, and therefore why a corpus would grant a capacity whose misuse it goes on to charge. The rest of this entry takes up that question, and the answer changes the status of the charge.
...
So the grant of speech is unfenceable at the register that carries it. **Taḥrīf is therefore what an unfenced grant does over time, not a failure that better custodianship would have prevented.** This is why the charge never functions in the corpus as surprise, and why it is levelled at communities of learning rather than of ignorance. The faculty that produces faithful transmission is the same faculty, and it comes without a governor.
