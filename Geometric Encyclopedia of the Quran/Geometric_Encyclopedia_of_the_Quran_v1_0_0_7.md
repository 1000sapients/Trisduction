...
<!--
GEOMETRIC ENCYCLOPAEDIA OF THE QURʾĀN · Seed corpus · Version 1.0.0.7
...
CN-PSP-TAHRIF-GRANT-01 at SC-088, Maʿiyya from CN-PSP-PLUS-ONE-MASTER-01 at
SC-093, and Companion from CN-PSP-QARIN-COUPLING-01 at SC-105, all in the
theological register. Cite identifiers only; SPINE_INDEX.md
resolves them to the current codex source.

Census: 7 entries, 4 major, 3 standard, 0 redirect stubs. This edition admits Companion at major tier, the third entry
with both gates green and the first whose spine and entry cycles both closed SEALED rather than passing; it opens the
unseen-apparatus bucket, seeds sūra forty-three and reaches three further unseeded sūras, and arrives through the quota
channel by architect commission with the provenance stated on its face. The odometer moves from six to seven.

Two corrections travel with this edition and neither moves the odometer. The version strings in this header stood at
1.0.0.3.1 while the file was named for census six and the census line read six; the filename and the census line were
right and the header strings were stale by three editions, and they are corrected to 1.0.0.7 here. And the range line
advances from A-T to A-T with letter C now occupied, the fourth opener in the corpus.
...
<!-- ADMISSION: date=2026-09-04; spine=SC-105; spine-audit=PASS;
     entry-check=PASS; entry-audit=PASS-qarin-r7; rounds=7; tier=major;
     frame=corpus; bucket=unseen-apparatus;
     depth=resident;
     channel=quota; origin=SC-105 -->

<!-- ENTRY LEDGER · MUST NOT RENDER
filing key=companion, from the English head as the corpus files it, not from the transliteration
tier=major; ~2100 words, inside the 1950-3250 band and well under the 3250 ceiling
all ten movements present, which is major's own signature in the extent table
tier moved from standard after four compression passes across two drafts; every remaining
  sentence carries content, so the surplus is real extent and not slack
spine=SC-105 CN-PSP-QARIN-COUPLING-01; cycle=SEALED-ROUND r14; coverage 6/6; controls 3/3 SELF
entry cycle SEALED-ROUND at r7, coverage 6/6, controls 3/3 SELF, one auditor erratum at r4
mechanical validation 27 checks, zero FAIL zero WARN
counts executed against Uthmani plain text, 6236 verses, sha256 6a9010407fb8be4a:
  qarin singular 6 verses / 7 tokens; quranaa 1 verse / 1 token; 8 tokens total
  qayyada exactly 2 sites (41:25, 43:36); atid exactly 2 sites (50:18, 50:23)
cross-scripture run at all three placements: literature furniture, cognate NEGATIVE, parallel DIVERGENT
CB_NAVIGATOR page-location for 1 Sam 16:14 owed; loci verified in text
bibliography Primary-only; literature survey is of the primary and secondary works themselves;
  the reference works are consultation apparatus only and do not appear on the page;
  EI2 was the route to the citations, they are uninspected, and that is stated on the face
open question inherited from spine: withdrawal of the assignment when the condition lapses
seal ratio: corpus goes 7/7 if admitted; does not serve the break
backlink sweep: not run
-->

# Companion (qarīn, قرين)

A *qarīn* is a coupling rather than a kind of being. The word names the relation of being yoked to someone, and the Qurʾān fills that relation once with a human peer and elsewhere with a *shayṭān*, assigned under a condition at Q 43:36 and had under a condition of a different shape at Q 4:38. The allotted companion arrives after the person's own turning away, works on how the world looks to him rather than on what he has been told, is never described as writing anything, and at the Reckoning is not removed but read differently. The word appears eight times across seven verses, and the order in which the assignment is described, stated once and stated grammatically, governs most of what was later built on the figure.

## In the literature

The exegetical tradition settled early on a two-companion reading, a devil and an angel attending each person, one urging evil and one good, which al-Ṭabarī records as the oldest position and which al-Bayḍāwī is reported as following. The lexica gloss the word at its widest by *muṣāḥib*, and al-Bayḍāwī by *khidn* at Q 41:25, a verse away from the assignment itself. He is undecided at Q 50:23 whether a devil or an angel is meant and certain of a devil at Q 50:27, so an identification the later handbooks close without argument is left open by the same commentator who is reported as following the oldest position at the neighbouring verse. Al-Ghazālī gives the fullest ethical treatment, in the *Iḥyāʾ ʿulūm al-dīn* at the *Kitāb ʿajāʾib al-qalb*, where the companion belongs to the warfare of the heart rather than standing at the shoulder as a creature. The collections carry the material in bulk, Muslim on the Prophet ﷺ and his own companion, with parallels in Ibn Ḥanbal and al-Dārimī, and al-Shiblī's *Ākām al-marjān*, a treatise on the jinn at large, carries a great many traditions bearing on it.

The lexicographers preserve a use older than any of this. A *qarīn* could be the *jinnī* who attended a poet and brought him his verses, for which Goldziher is the standing citation, and the usage was later transferred to the angel who brought revelation, so the companion begins as a figure of communication and acquires its moral fork afterwards. In the modern folkloric register the companion is read as a survival of animism, and there the conditional clause disappears from view. None of the exegetical or modern material named here has been inspected against clean editions.

What has not been asked is what the companion is downstream of: every treatment takes it as a standing fixture and reads a person's conduct from it, while the Qurʾān describes the order the other way at the one verse that assigns it.

## The word

The root q-r-n joins. *Qarana* is to couple, *qirān* a conjunction, and *qarn* is both the horn, a paired projection, and the generation, a cohort coupled in time. Underneath all of them is the yoke, the bar that makes two draught animals one team, and every abstract sense is that bar seen from further off.

The root carries two senses that do opposite things to whatever they take hold of, and the split explains much of the later confusion. On one side *qarīn* is the one joined to a person, an associate; on the other, *muqarranūn* at Q 14:49, Q 25:13, Q 38:38 and Q 43:13 are the bound, coupled in irons and against their will. Association in the first case, constraint in the second, so when the tradition oscillates between a companion who tempts and a companion who holds, it is hearing both senses in one word.

The coupling sense does not survive into the earlier scriptures. Hebrew *qeren* and Aramaic *qarnā* keep the horn and drop the yoke, and the Hebrew for a yoked pair is *ṣemed*, from an unrelated root. The Arabic sense is therefore not an inheritance and there is no lexical bridge to trace one across. What does cross is the idea, carried by words of its own, which is a more interesting result than a shared etymology would have been, since borrowing cannot account for it.

## The verses

Seven verses, eight occurrences. Q 4:38 places the companion where wealth is spent to be seen by people, *riʾāʾa n-nās*, using the word twice in one clause. Q 37:51 gives it to a human being, an associate in this world who mocked the resurrection and whom a man in the Garden recalls, which settles that the word is not reserved: whatever else a *qarīn* may be, it is not a species. Q 41:25 carries the only plural and the only description of what these companions do, *fa-zayyanū lahum mā bayna aydīhim wa mā khalfahum*, they adorned for them what lay before them and behind them. Q 43:36 carries the assignment. Q 4:38 is conditional too but of a different shape, a man having such a companion rather than being given one, with no assigning verb and no turning named, so it neither supports the reading below nor troubles it. Q 43:38 carries the wish for distance at the arrival, and Q 50:23 and Q 50:27 two speeches at the Reckoning, one presenting what is ready and one refusing the charge.

Two smaller patterns are worth noticing. The verb that assigns the companion, *qayyaḍa*, occurs at exactly two places in the whole Qurʾān, Q 41:25 and Q 43:36, and is used for nothing else. And *ʿatīd*, ready to hand, occurs at exactly two places, Q 50:18 and Q 50:23, both inside Sūrat Qāf, once of the watcher who sits by a man and once of what the companion holds.

Sūrat Qāf lays the subject out along one line: at verse 16 the nearness of Allah ﷻ, nearer to a man than his jugular vein, with the whispering in that same verse attributed to the man's own self, *naʿlamu mā tuwaswisu bihi nafsuh*; at verses 17 and 18 the two receivers seated at right and left; at verse 22 the covering taken off; at verses 23 and 27 the companion speaking. The whispering is inside the man seven verses before the companion says anything.

## How the pieces fit

Three things are at work and they sit apart cleanly.

There is the coupling itself, a relation with two ends, shown to be a relation rather than a class of creature by being filled once with a human being.

There is what the companion does, at one end of the relation only. The operation described at Q 41:25 is applied to what lies before and behind a man's seeing, his field of view and not the text he was given. The Qurʾān's vocabulary for failed reception runs the same way throughout: a seal and a film at Q 2:7, a stamp at Q 7:100, locks at Q 47:24, rust at Q 83:14, a covering at Q 50:22, every one laid on the receiver, against a message described as clear and four times in one sūra as made easy for remembrance. Q 22:52 is the single place where an adversary reaches the message instead of the receiver, and the same sentence cancels the casting and makes the verses firm, so the attempt is on record and nothing of it remains.

And there is what binds the two, which is a condition. Q 43:36 reads *man yaʿshu ʿan dhikr al-Raḥmān nuqayyiḍ lahu shayṭānan fa-huwa lahu qarīn*, whoever turns blind from the remembrance of the Most Merciful, We assign to him a devil, and he is a companion to him. The construction is conditional, *man* with the jussive *yaʿshu* setting the condition and the jussive *nuqayyiḍ* answering it, so the assignment does not merely follow the turning but depends on it. In the plainest terms the companion is an output and not a free input, and a reading that treats it as something a person simply has adds a variable the sentence does not contain.

Two things follow that are unavailable if the companion is taken as a fixture. The companion's words at Q 50:27, *mā aṭghaytuhu wa lākin kāna fī ḍalālin baʿīd*, I did not make him transgress, he was already in far error, are left standing in the text without correction, and they are left standing because they are true. And the companion amplifies without originating, which Sūrat Qāf says on its own account at verse 16 by locating the whispering in the *nafs*.

An older scripture makes the same move with different words. At 1 Samuel 16:14 the spirit of the LORD departs from Saul and an evil spirit troubles him, the troubling spirit arriving after the departure and never before it. The shape is the same and the agent is not: there both the departure and the sending belong to the LORD, while at Q 43:36 the turning is the man's and the assignment answers it. Acts 12:15, *it is his angel*, carries the personal-companion idea with no moral fork in it at all. Neither passage has a companion whose arrival turns on the person's own act. Nothing in the comparison bears on the standing of either text.

!seal ⟀ GOL

## Two things under one name

The recording keepers are described over human beings as such. *Wa inna ʿalaykum la-ḥāfiẓīn, kirāman kātibīn*, over you are keepers, noble scribes, at Q 82:10 and 11, where the address is set four verses earlier by the vocative *yā ayyuhā l-insān*, O man; and the two receivers at Q 50:17 are seated one verse after *wa laqad khalaqnā l-insān*, and We created man. The companion of the second kind is described under a condition and in no other form anywhere in the book. The keepers therefore reach a reader who has never turned away, since the address reaches him, while the assignment reaches nobody until the condition is met. The familiar teaching that every person has a *qarīn* is an addition to what the verses say, and worth recognising as an addition.

The second distinction is between presenting and writing. The Qurʾān gives the writing to the noble scribes and to nobody else; it does not say the companion cannot write, it simply never gives it that office. And what the companion holds out at Q 50:23 is nowhere joined to the scribes' record. The two verses share *ʿatīd* and stand five apart, a link between two words rather than an identification of two objects, so what is being presented stays open, as does whether the speaker at Q 50:23 is the one who speaks at Q 50:27.

## The tradition

The nearest thing outside the tradition is the teaching of the two spirits in the Qumran Community Rule, where a spirit of truth and a spirit of deceit are set over a man, which is the two-companion scheme without the conditional, carried here at witness grade and uninspected. The received taxonomy itself is mapped and not adopted, and it gives two results running opposite. It keeps the two companions apart from the recorders, which the verses give on their own. And it supplies the universality, which they do not. Beside those stands its one case of removal, the report that the Prophet ﷺ had such a companion and that it submitted, after which it commanded him only good, a case that is by its nature interior and that no verse states.

## What the word will not carry

None of this settles anything about any person, and nothing here supports a practice, a diagnosis or a rite. It is not a doctrine of fate: the assignment is conditional at Q 43:36 and the rust that rises over a heart at Q 83:14 is what its owner was earning, so what accumulates is not what is imposed. The universality belongs to the reports and is not read back into the verses. The Qurʾān never calls an angel a *qarīn*; it says angels, keepers, noble scribes, a watcher ready by him. The reading inverts if the companion is treated as the cause of the turning, which the conditional rules out. And the assignment at Q 43:36 is an act of Allah ﷻ described as the Qurʾān describes it: what is observed here is the grammar of a sentence and never its Subject, who is no term in any scheme.

## What it is confused with

The live confusion is with the recording angels, and it is an old one. The recorders write and are described over everyone; the companion is assigned under a condition and given no writing to do, so running them together produces a universal tempter the text does not describe and loses the condition it does. A second confusion runs the other way, between the assigned companion and the human associate of Q 37:51, the word covering both and the assignment only the first.

One question has no answer in the text. Whether the companion is withdrawn when the condition stops holding is never said. The Qurʾān gives no clause of withdrawal, and the one case of removal in the tradition is a private matter reported by others.

!seealso Q 43:36 · Watchfulness (murāqaba) · Heart (qalb) · Book (kitāb) · Guessing at the Unseen (rajm bi-l-ghayb) · Guidance (hudā)

@ M. F. Islam

Bibliography
Primary: al-Qurʾān, Uthmani plain text, 6236 verses, file digest sha256 6a9010407fb8be4a; all counts given above were run against it. Biblical loci at 1 Samuel 16:14 and Acts 12:15.
Secondary: none. The article argues from the Qurʾān and from the root. The exegetical, lexical and modern material surveyed above was located through a reference work, has not been checked against clean editions, and is carried as unverified; nothing in the reading rests on it.
