...
Module X (Public Label Map and PDF Hardening Set) fires on every PDF deliverable, without a trigger phrase. It renders native codex identifiers as short public labels read from the frozen append-only registry at `publication/PUBLIC_LABEL_MAP.json`, never re-derived at render time, prints a Coordinate Index in the end matter, and applies six build-level locks: glyph coverage assertion, verdict legend, tier chips, automatic table span, the `:::receipt` reproduction box, and inline-mathematics no-break. Short labels are barred at Tier 0 and Tier 1 exactly as native identifiers are.
...
Step 8b. Run Module X: substitute short public labels from the frozen registry, emit the Coordinate Index into the end matter, and apply the PDF hardening set. Write the substitution log to the working directory.
...
Every PDF deliverable: Module X short labels read from the frozen registry at `publication/PUBLIC_LABEL_MAP.json`, no register marker carried forward, a Coordinate Index in the end matter, labels barred at Tier 0 and Tier 1, and the six hardening locks, glyph coverage assertion, verdict legend, tier chips, automatic table span, the `:::receipt` box, and inline-mathematics no-break.
...
**The Coordinate Index.** Every PDF carrying short labels prints a Coordinate Index in the end matter, full width below the columns, beside and before References. Three columns, label, title, native identifier, in label order, no prose. Combined with the Module VIII master reference and its DOI, every short label resolves to a public artifact without the body ever printing a native identifier.
