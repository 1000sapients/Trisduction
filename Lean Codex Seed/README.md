...
|---|---|---|---|
| `TRISDUCTION_PSP_Integrable_v1_0.lean` | 1.0 | 6bcd6a2c92d754b3 | **The PSP integrable part**: SECTION 19 for the master. P1-P9 and P22: the orientation-blindness of the lock scalar, the quaternion / octonion / sedenion cluster, the cascade and token census, the two-group law. 48 declarations, 41 on propext, 7 axiom-free, no posit; its 48 guards generated from the merge probe. Compiles appended to the v1.1.3 body at exit 0; the sedenion-pair mutant exits 1 against the merge. FORGE cycle psp2 sealed |
...
| `TRISDUCTION_Master_Codex_v1_1_3_full.lean` | 1.1.3 | bc48a75327f6baa5 | **Current, full. The sealing round.** D4a closed over Int (the adjugate identity for every integer matrix, both orders), the finite leg of D9 closed on every enumerated carrier (both proofs compiled on a second substrate before seating), the free leg restored to `one_bit_freedom_priced`, the owed count rendered for every m ≥ 1 with zero marked in both renders and injectivity decided, `halt_reports_owed` and `owed_zero_is_unchannelled` proved, K4's table written as the construction it is (lifts mod 210), the two halt records banner-marked NOT A PROOF, the NOMOS reading separated from the NOMOS theorem, one false audit refused at the foot with its mechanism. 480 guards, exit 0 |
| `TRISDUCTION_Master_Codex_v1_1_3.lean` | 1.1.3 | 1d8c07dfd2f11f37 | **Current, barebone.** Program only; same declarations, same 480 guards, byte-identical output |
