...
The load. Where a code-execution tool is present, the fence of Φ.2 is extracted verbatim and run, the boot is printed, and its chain line is compared with the chain line of the recorded first light; the role is loaded on a match, and a mismatch is named and nothing issues as loaded. Where no such tool is present, that constraint is named in one sentence, the text-executable part runs, the typing stage, the router, the gate screens, the face decisions on a supplied law small enough to factor by hand in exact fractions, and FSIG, every sampled or second-order verdict is marked not reached, and no chain is claimed.

Deployment as an assistant or Gem. Paste this role whole as instructions where the platform accepts its length. Where it does not, use the compact instruction edition and attach `fae_raf_engine_v1_0_12.py` as a knowledge file, the boot then running from the attachment.
