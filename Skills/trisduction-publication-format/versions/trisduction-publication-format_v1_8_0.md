...
version: 1.8.0
description: "Master publication discipline for written documents: content architecture, voice, and a locked EB Garamond copper-accent visual contract. Use for any paper, report, treatise, codex, textbook, manual, scripture, or reference work as PDF, DOCX, or Markdown. Triggers: 'create/forge a paper,' 'make a textbook,' 'deliver as pdf,' 'save as docx,' 'visual edition.' Editions: Apex Pristine house paper; Journal ([EDITION:JOURNAL]); Math Journal ([EDITION:MATH_JOURNAL], [FORMAT:MATH_PDF]); Textbook ([EDITION:TEXTBOOK_D]); Scripture ([EDITION:SCRIPTURE]; 'tafsir pdf'); Encyclopedia ([EDITION:ENCYCLOPEDIA]; 'reference work'). Blog ([FORMAT:BLOG]; 'medium,' 'substack') emits Markdown. Upgrade ([UPGRADE]) keeps a paper's spine. A Register-Transit Filter keeps confessional content out of secular artifacts (REGISTER:SECULAR default). A Falsification Gate (I.B.1) caps criteria at three, typed for necessity. A Positioning Discussion (XI) closes eligible scholarly papers on a table of prior positions ([POSITION:ON/OFF])."
---

# TRISDUCTION PUBLICATION MASTER · v1.7.2

**v1.8.0, the render-completeness law.** One module added, XII, after three builds of one paper shipped truncated while the builder printed success. WeasyPrint's `column-span: all` can silently drop the remainder of a document: no exception, no warning, a valid PDF of the surviving pages and a page count that looks like an answer. A 44,000-word paper ended mid-sentence in Section IX with no Conclusion, no References, and no positioning tables, and it passed every gate in this file, because **every gate here checked the source and none checked the artifact**. Four properties are recorded because each defeated a repair: the fault is silent; it migrates with word count, so a source completing at 43,400 words truncates at 44,200 and yesterday's success proves nothing about today's; it is version-dependent but not version-fixed, 65.1 carrying further than 69.0 without removing it; and CSS does not touch it, with `break-inside: avoid`, `break-before: page`, fewer rows, shorter cells, more parts, and prose between the parts all tested and none helping. The only length-independent configuration is no spanning element at all. The law: **a render is verified by its end, never by its exit code**, with a three-check probe on the built file, terminal string, page count against the last good build, and Conclusion and References present by name, any failure meaning not a deliverable. Spanning is capped rather than banned: long, table-dense, or sealed documents render all tables at column width, because completeness outranks width and a build whose success flips with every added paragraph cannot carry a seal. **The finding worth carrying: a tool reporting success is not evidence the work is intact, which is the artifact-side twin of the subtractive check the automation skill learned at L9 after a register was silently emptied. Ask the artifact, not the exit code.**

**v1.7.2, the Blog edition made mandatory after a sealed cycle.** One clause in Module VII and one in Step 10, wired to the closing-card contract of `trisduction-audit-cycle` v2.3.3. A paper that has passed its FORGE cycle now leaves the session as three files and never two: the rendered PDF, the Markdown master, and the Blog edition rendered from that same sealed master under Module VII, one version and one date across all three. The reason is the reading surface: the PDF is the citation artifact and the master is the editing artifact, and neither is what a reader on Medium or Substack can open, so a sealed paper without its Blog edition is sealed and unread. The Blog file renders the sealed text and adds no claim, so it owes no cycle of its own; the Register-Transit Filter checks it at the Blog register on the way out. The finding worth carrying: **a deliverable set is a set, and a rule that names two of three files has named the one a reader will never see.**

**v1.7.1, the credential seated as law.** One clause added at §III.B and mirrored into the Journal and Math Journal front-matter clauses and into Gate 1, and it records a miss that the file itself caused. The academic credential stood in this file only as an exemplar value, `author_name: Mohammad F Islam, PhD`, inside the Apex Pristine front-matter template, a default in one edition's example and a rule nowhere; the Journal clause named `author_line` with its superscript markers and no credential, so a Journal paper composed against its own clause went out under the bare name. **A default in one edition's exemplar is not a rule for another edition**, and the fix is a cross-edition clause with a gate rather than a firmer example. The credential is now part of the name entity on every scholarly edition, Apex Pristine, Journal, Math Journal, Textbook, and Blog, `Mohammad F. Islam, PhD` with the affiliation marker after the credential where the edition uses one, and absent by design at the devotional register, where the Scripture edition's humble-servant line stands because that register is witness and not authority. Gate 1 now fails a scholarly byline without it before any build. The finding worth carrying: **the field table said Author name line and the exemplar showed the credential, and a scribe reading the table as the law and the exemplar as decoration reproduced exactly the gap between them.**

**v1.7.0, the positioning discussion seated in the template.** One module added, XI, and one clause each in §I.B Section 8 and in the Execution Pipeline wired to it. The order came from a paper whose Discussion was made to close on a single table, one row per prior position the body engaged, five columns, the position, what it holds, what the paper does with it, the relation in one defined word, and the evidence in one word, and the table did more for the paper's standing than any paragraph of advocacy could, because it stated the contribution as exactly the set of relations it carried and nothing wider. The module makes that close a standing part of the generic template for the papers that can bear it, and only those: register secular, a field with a citable literature, and mass in hand, an executed finding with receipts or a synthesis the literature does not hold, the paper already carrying its own fence on what it does not contribute. Eight relation words are defined once and used as defined, and the two that inflate most easily are gated hardest: contradicting is denied on executed data and never on argument alone, and superseding is permitted only where a named theory's central claim is retired on executed data at the same register, else barred and the legend says so. The table is registered as an audit claim with a falsifier, so the cycle prosecutes its fidelity to the body row by row. The finding worth carrying: **a comparative paper sells itself by the only means the discipline permits, stating where it stands to each position in words whose senses are fixed, and a relation word applied outside its definition is the anchor inflation the Shield forbids wearing a table's authority.**


**v1.6.0, the falsification gate seated at genesis.** One module section added, I.B.1, and two readings of the twelve-section template corrected with it. The order came after a full upgrade-and-audit run in which the paper's own falsification register had to be repaired during the audit, and the point of the order is that no later audit should have to: **a criterion caught at audit has already been printed, cited and defended, while a criterion caught at genesis was never written.** The rule is few and tight, two criteria and three at the ceiling, each tested against geometric and mathematical necessity before it is drafted. The standard is lifted from `APEX-PSP-PHYSICAL-SEED-01` §VI clause 5 and its gravity seed rather than invented here: one falsifier, forced by the residence's own reading, seed-native, binding on two named conditions, aimed at one coordinate, running against the author. Five tests run in order, necessity first and hardest, then genealogy on the falsifier's own terms, then blast radius, then the anchor loop barred outright, then conditions and sharpness. Two corrections to this file's own prior reading came with it, and both were defects rather than preferences. Section 7 said **three to five** predictions, which is more attack surface than any paper needs to volunteer. And §I.B made Section 7 a co-anchor with Section 3 at forty to fifty percent of body length, **which put the paper's weight on the one section whose intake nobody screens**; the weight moves to Sections 3 and 6 where the geometry actually lives, and Section 7 comes down to four to eight paragraphs. What survives untouched is the arithmetic discipline that was already right, no arbitrary human-fitted constants and every threshold parameter-free or derived from a single topological invariant, which is what a forced criterion looks like read from the arithmetic side; the two halves were always one rule and the gate supplies the half that was missing. One clause prevents a deadlock with Module VI: **cutting a falsification criterion is never a concession and needs no mechanism**, because a falsifier is an invitation and not a claim. The finding worth carrying: **a discipline that screens every term in the body and no term in the falsifier has screened the paper and left the door open**, and the misfire record proves it, four clauses re-typed in one day and one coordinate hard-deleted on an anchor loop when a single orthodoxy import retired.

**v1.5.5, the stamp repair.** No renderer changed and no discipline moved. Three metadata defects close and one guard is added, and all three were produced by the patch passes that preceded this one rather than by any installation. The front-matter `version` field stood at 1.5.1 while the masthead read v1.5.4, so three consecutive passes moved one authoritative string and left the other, which the propagation clause above names as a defect by definition. The changelog table read v1.5.3 above v1.5.4, because the v1.5.4 row was inserted against the v1.5.2 row as its anchor and landed beneath its own predecessor. And v1.5.3 and v1.5.4 shipped table rows carrying no narrative stanza, against the one-stanza-per-pass clause, which is the same failure a third time. **The finding worth carrying: a patch pass touches the place it is thinking about and misses every other place that states the same fact.** The repair is therefore a mechanical comparison at the close of the pass rather than a firmer statement of a rule that was already stated. The body was re-verified whole under this pass and is intact: the `!seealso` parser, the comment strip at the head of `parse()`, and III·E.14 all present, code fences balanced, encoding clean, ten modules and five sub-editions standing.

**v1.5.2, the uniform grid.** One clause added, III·E.14, and it records a repair that was wrong in kind before it was right. Vocalised Arabic overflows the Encyclopedia body line box and collides with its neighbours; the first repair opened the leading on any paragraph carrying Arabic, which stopped the collision and destroyed the uniform grid the dense reference page exists to hold. The seated rule instead sizes inline Arabic to the line box and strips its tashkil, the vowels being carried by the transliteration that follows every run in the house entry form, and licenses vocalisation at a display form that owns its own line. Noto Sans Math is named in the serif stack rather than reached by fallback. The finding worth carrying: **a fix that removes a symptom by breaking the invariant the page exists to hold is not a fix**, and the tell is that it repairs one paragraph and leaves its neighbours untouched. Both the defect and the false tofu report that accompanied it were invisible in the source and obvious on a rendered page.

**v1.5.1, the description cap.** A repair, and the failure it records is worth more than the fix. The front-matter `description` carries a hard ceiling of 1024 characters imposed by the loader, and v1.5.0 pushed it to 1201 by appending the Encyclopedia triggers to a field already sitting near a thousand. The field was therefore one edition away from breaking before this pass touched it, and the pass that broke it is not the pass that caused it. **The description is now compressed to fit with headroom, and III·E.13 fences the cap** as a counted authoring rule rather than a thing rediscovered at install. The finding worth carrying: **a limit with no stated number is a limit nobody is measuring against**, and the count audit of v1.5.0 caught every stale count in the body while missing the one field that was capped, because the audit enumerated what the file said and not what the loader would accept.

**v1.5.0, the reference register and the self-governed file.** Two additions and the second is the one that had been owed. **Module III·E, the Encyclopedia edition**, is the sixth document renderer and the only one whose unit is an entry rather than a section: a royal-octavo two-column reference page filed alphabetically, addressed by a guide-word running head that takes the outer column while the folio takes the inner, with letter openers between the runs. Its page contract was measured from a mature six-volume reference set by coordinate extraction rather than estimated, and the two findings that carry its identity both invert the ordinary book convention: the running-head elements are centred over their columns rather than flush to the margins, and the folio hugs the gutter so the guide word sits where a thumb finds it. **R.4 now governs this file**, which is the correction. R.4 stated version propagation as law and scoped it to register writes, so the one governing document in the stack that a scribe reads as current law was the one document the law did not reach. The finding worth carrying, and it is the v1.4.0 finding arriving one layer up: **a rule that exempts its own carrier is not a rule, it is a preference the carrier happens to hold**, and audit symmetry is the only thing that catches it, because nothing downstream ever will.

**v1.4.0, the codex boundary.** One rule added and it closes a gap the register had been paying for by hand. Module II carried the whole Silent-Integration discipline and scoped every clause of it to *the paper*, while Module IX already fired on master-codex incorporation without exception. Nothing therefore governed revision archaeology at the codex boundary, and seated coordinates were accumulating scribal error rows in their bodies because no rule barred it. **II.C now runs Module II at boundary two**, with one routing clause: errata go to the audit ledger and to the codex changelog stanza, never into a card body, and Gate 1's scan extends to card bodies before any seating. The finding worth carrying: **a discipline scoped to one boundary is not a discipline, it is a habit that happens to hold there**, and the register pays for the ungoverned boundary in exactly the coin the governed one was protected from.

**v1.4.0 is also the first edition of this file to carry a version string, and the omission was structural rather than careless.** Module II strips version markers from artifacts, and the file applied its own artifact rule to itself. The rule inverts by surface exactly as the sibling governance layer states it: a published artifact wants no history and a governing document wants all of it, because the next scribe reads the governing document as current law and needs to know which law he is holding. Prior editions were unversioned. The three copies filed under `publication/` carry filing counters and not skill versions, and the numbering starts above the highest counter so the two schemes cannot collide.

Single master discipline. Ten integrated modules and five edition variants. One artifact pipeline.
...
## Version Propagation of This File

R.4 states version propagation as law and this section is the clause that points it at its own carrier. The rule inverts by surface, exactly as the v1.4.0 stanza records: a published artifact wants no history and a governing document wants all of it, because the next scribe reads this file as current law and must know which law he is holding. So this file is version-propagated in full, and no clause of R.4 is relaxed for it.

**The version string is carried twice and both are authoritative.** The `version` field in the front matter is the machine-readable value and the masthead string is the human-readable one, and a pass that moves one and not the other is a defect caught before delivery. The front-matter field is what a resolver reads; the masthead is what a reader reads; they never disagree.

**The check is mechanical and runs at the close of every pass, because the rule by itself has now failed three times running.** v1.5.2, v1.5.3 and v1.5.4 each moved the masthead and each left the front-matter field at 1.5.1, so a resolver reading the machine-readable value would have seen a three-edition-old file reporting itself current while the human-readable value read true. Before delivery, read both strings and compare them; a pass that cannot show them equal is not delivered. And **a new changelog row is prepended at the head of the table, never inserted against a named neighbour.** The v1.5.4 row was anchored on the v1.5.2 row and landed beneath v1.5.3, so the table read out of order for two editions. Anchoring on a row is a positional assumption that decays as the table grows; the head of the table is the only anchor that stays true.

**Every write is a new version file.** Re-resolve the highest version present in the skill folder, author the next, and create it at a new path. Prior editions are never touched, never overwritten, and never removed, so an interrupted write cannot destroy the current discipline and the last good edition remains the fallback until the new one is confirmed. Resolution is by numeric tuple compared component by component, never by string sort, so 1.10.0 resolves above 1.9.0; a CURRENT pointer, where one exists, is advisory and never overrides the highest version actually present.

**Bump semantics follow R.4 without amendment.** Patch for a small edit, a clarified clause, a corrected constant, a repaired directive. Minor for a new module, a new edition, or a new section. Major for a restructure that moves the module ladder or breaks an authoring grammar. An edition adding a renderer is therefore always at least a minor, which is what v1.5.0 records.

**The version rides the bundle, never the file inside it.** The loader requires the folder named `trisduction-publication-format` and the file named `SKILL.md`, so the version stamp goes on the distributed bundle, as `trisduction-publication-format_v1_5_0.skill`, and on the two version strings within. Stamping `SKILL.md` itself would satisfy the propagation law and break the loader, which is the one place this discipline yields to a mechanical constraint, and it yields by moving the stamp rather than by dropping it.

**History accretes by stanza, one per pass, never by page.** Each edition prepends a single stanza to the block above naming what was added, what was corrected, and the mechanism, in the compression the Harvest Distillation Law licenses. The dialectic that produced the pass routes to the session record and the chronicle. This file is a history by function and carries its evolution in full, which is the stated exemption from the artifact strip of Module II and not a lapse in it.

**The front-matter `description` is capped at 1024 characters and the cap is counted, not estimated.** It is the field a loader validates and the field every routing trigger lives in, so it grows on every edition that adds a renderer and it is the one place an additive pass silently exceeds a limit the body never mentions. Compose it to roughly nine hundred and leave headroom for the next edition; measure the string before delivery rather than after install. A field that will not compress has usually accumulated aliases rather than triggers, and the repair is to cut redundant aliases, never to drop a selector, since a missing selector costs a route while a missing alias costs nothing.

**The pass closes on a count audit.** Any edition that adds or removes a renderer re-counts every place the file states how many there are: the module ladder, the co-fire precedence sentence, the tagline, the skip lists of Module VIII and Step 8, and the structural exemptions of the twelve-section template. The audit also re-measures the front-matter `description` against its 1024-character cap. A stale count is the characteristic failure of an additive pass, because the addition is visible and the arithmetic elsewhere is not.

| Edition | Bump | What moved |
|---|---|---|
| v1.7.2 | patch | The Blog edition made a mandatory third deliverable after every sealed FORGE cycle on a paper, rendered from the sealed master under Module VII; Step 10 and Module VII wired; the audit-cycle closing card carries the matching clause. |
| v1.7.1 | patch | The academic credential seated as a cross-edition law at §III.B, mirrored into III·J.7 and III·M's front-matter clauses, with a Gate 1 check; absent by design at the devotional register. |
| v1.7.0 | minor | Module XI, the Positioning Discussion: eligibility gate on register, field, mass, and the fence; eight relation words defined and gated with superseding gated hardest; the standing table's five fixed columns and one-word evidence; the fidelity law with its audit claim; placement as the close of Section 8; page-fitting spanning parts; the handout derivative with its own cycle. §I.B Section 8 and Step 3b wired to it. |
| v1.6.0 | minor | Module I.B.1, the Falsification Gate, seated at genesis: two criteria and three at the ceiling, five necessity tests, the Physical Seed standard, the anchor-loop bar. Section 7 retyped from three-to-five to two-to-three and from co-anchor to four-to-eight paragraphs; §I.B weighting moved to Sections 3 and 6. Abstract, Conclusion and Step 3 wired to the gate. |
| v1.5.5 | patch | Front-matter version synced to the masthead after three passes moved one and not the other; the two swapped changelog rows reordered and prepend-at-head made the rule; stanza added for the pass. Body verified intact under this pass, no renderer and no discipline changed. |
| v1.5.4 | patch | III·E parses `!seealso`: seed verse set first in small caps, targets interpunct-separated, a rule above the tail, and the token added to the bibliography-continuation guard. The directive was seated in geometric-encyclopedia-quran v1.5.0 without the renderer that must read it, so the raw token printed on the page. A source directive and its parser are one change. |
| v1.5.3 | patch | Hard rule R at parse(): HTML comments stripped before parsing, so scribe-facing internal blocks never surface in a rendered deliverable. The leak was invisible to inspection and was caught only by post-render text extraction. |
| v1.5.2 | patch | III·E.14 uniform-grid law: inline Arabic unvocalised and size-matched, vocalisation at the display form, paragraph leading overrides barred, math face named in the serif stack. |
| v1.5.1 | patch | Description compressed under the 1024-character cap. Cap fenced in the propagation section and in III·E.13. |
| v1.5.0 | minor | Module III·E, the Encyclopedia edition. R.4 applied to this file. Module count corrected. |
| v1.4.0 | minor | Module II run at boundary two (II.C). First edition to carry a version string. |
| pre-1.4.0 | — | Unversioned editions. Filing counters under `publication/` are not skill versions. |
...
An alphabetic reference volume routes to the Encyclopedia edition. Triggers: encyclopedia, encyclopaedia, reference work, alphabetic entries, dictionary pdf, lexicon volume, the selector [EDITION:ENCYCLOPEDIA] or edition: encyclopedia, and any deliverable whose unit is a headword filed alphabetically rather than a numbered section. The royal-octavo two-column reference page renders, guide word over the outer column and folio over the inner, letter openers between the runs. This route is more specific than Journal and less specific than Scripture: a verse-by-verse tafseer stays Scripture even when it is called an encyclopedia, while an alphabetic reference work of Quranic terms routes here.
...
The seven never co-fire on one deliverable. When two triggers appear, the more specific wins in the order Scripture, then Encyclopedia, then Textbook, then Math Journal, then Journal, then Apex Pristine, so a Quranic tafseer renders as Scripture, an alphabetic reference volume as an Encyclopedia, a codex paper as a Textbook, an equation-dense paper as a Math Journal, and an external prose paper as a Journal; an explicit selector overrides the order entirely. The Medium/Substack/Blog format (Module VII, [FORMAT:BLOG]) is orthogonal to this routing: it is a delivery target rather than a page renderer, so when it is requested it supersedes the PDF editions for that deliverable and emits a copy-paste-safe Markdown file only, applied to the authored content whether codex, paper, or scripture.
...
Step 3. Compose content on the twelve-section Plenous template (Module I.B) at the calibrated length (Module I.C), clearing every falsification criterion through the gate of Module I.B.1 before Section 7 is drafted. Rendering-only jobs skip to Step 6.
Step 3b. Run the eligibility gate of Module XI. Where the paper clears it, draft the Positioning Discussion as the last run-in of Section 8 with its standing table, register the table as an audit claim with the falsifier of XI.4, and log the ruling in the working directory; where it does not, log the ruling and the section closes as before.
...
Step 10. Present artifacts via present_files in the order PDF, DOCX, MD, and, for every paper that has passed a FORGE cycle, the Blog edition MD rendered under Module VII from the same sealed master, mandatory and never optional. No postamble.
...
Mandatory structure for every Forge-mode paper at the mid-tier and fully-public tiers. The full twelve-section template is required for both the Apex Pristine mid-tier house paper and the Professional Journal fully-public paper, and neither tier may drop to a reduced structure; any mid-tier or fully-public paper carries all twelve sections. The Textbook, Scripture, and Encyclopedia editions are exempt and carry their own chapter, verse, and entry structures under Modules V, III·S, and III·E. Sections may be expanded but never omitted. Order fixed. **Section 3 (Barrier Analysis) and Section 6 (the structural core) carry the paper's weight**, and Section 3 alone accounts for roughly a quarter to a third of body length as the argument's first heavy anchor. Section 7 (Falsifiable Predictions) is deliberately **not** a co-anchor. An earlier reading of this template made it one, at three to five predictions inside a forty-to-fifty percent joint allocation, and that reading put the paper's weight on the one section whose intake nobody screens. The correction is stated in full at I.B.1 and enforced there: two criteria, three at the ceiling, each forced by the paper's own geometry, and the section short. A paper stands on its geometry and invites engagement through its falsifiers; it does not stand on them.
...
**Section 2 · Abstract.** Two hundred to three hundred words. Five sentences of structural content: (1) unsolved problem and why it matters; (2) specific structural gap in the standard model; (3) proposed novel mechanism; (4) the primary falsifiable prediction, which is the criterion the paper's geometry forces, with its confirmation threshold; (5) implication if confirmed. Fastest possible compression of the paper's geometry.
...
**Section 7 · Falsifiable Predictions.** The paper's invitation to external engagement, and deliberately short. **Two predictions, three at the ceiling**, every one of them cleared through the Falsification Gate of I.B.1 before drafting, and every one typed FORCED there, meaning the paper's own geometry entails it. An IMPORT never appears here; it goes beneath the register as a named tripwire at its honest grade, or it goes. Each surviving prediction is a **five**-part unit, the original four plus the typing the gate assigns. The Prediction: exact phenomenon to be observed, stated with a quantified threshold expressed as a dimensionless ratio, scaling exponent, phase-transition point, or absolute statistical significance level (≥5σ). No arbitrary human-fitted numerical constants. Every threshold parameter-free or derived from a single topological invariant. Method of Confirmation: specific instrumentation required including resolution thresholds. Real instruments, real observatories, real experimental techniques (JWST, DESI, Gaia, LIGO-Virgo-KAGRA, Euro-VLBI, AFM, smFRET, cryo-EM, MEG, ATLAS-CMS, ADMX-G2, JUNO, HyperKamiokande, DUNE, nEXO, LEGEND-1000, CMB-S4, LiteBIRD, Euclid, Roman Space Telescope, etc.). Expected Outcome: precise mathematical signal confirmation requires. Power-law exponent shift from one specified value to another, spectral plateau at a specific frequency, phase transition at a specified critical point. Null Hypothesis: exact empirical result that would instantly falsify the theory, stated with the same precision as the confirmation criterion, no wiggle room. Every prediction independently testable by at least two laboratories using orthogonal measurement modalities. **Necessity and Blast Radius**, the fifth part, one sentence each: the step of the paper's own geometry that forces this prediction, and the single named claim of this paper that its firing compromises, never the framework and never a coordinate the paper did not author. A prediction that cannot state its forcing step is an import and does not belong in this section. **Four to eight paragraphs**, down from the earlier allocation, because two forced criteria stated sharply are the whole of what this section owes a reader.

**Section 8 · Discussion and Implications.** Cascading consequences of the proposed solution. How resolution reframes adjacent problems. What previously mysterious phenomena become tractable. What new experimental programs become possible. Mild speculative latitude permitted, serving only to identify research directions. Even speculation grounded in the structural geometry of Section 6. Anticipated objections engaged on merit. Limitations identified honestly. Five to ten paragraphs. Where the paper clears the eligibility gate of Module XI, the section closes on the Positioning Discussion and its standing table, the last run-in of the section and the last thing before the Conclusion.

**Section 9 · Conclusion.** Three to five paragraphs: restatement of gap and proposed resolution; the primary falsifiable prediction, the forced one, with its confirmation threshold; explicit call to the experimental community to execute the proposed tests; the single most important open question remaining; single-sentence statement of the reframing that acceptance would entail. No ceremonial closure language and no doxology, the conclusion being the highest-risk locus for a devotional close; the governing law is Module IX and is stated once there, this line being its pointer.
...
## I.B.1 · THE FALSIFICATION GATE · hardened at genesis

The architect's standing order, given after a full upgrade-and-audit run whose own falsification register had to be repaired in the audit. **The gate is seated here, at genesis, precisely so that no later audit has to repair it.** A criterion caught at Phase 2 has already been printed, cited, and defended; a criterion caught here was never written.

**The proportion, which governs everything below.** Falsification is a minor aspect of the paper. It is an invitation to an external reader to engage, and nothing more. The paper does not stand on it. The paper stands on geometry, on the Tongue then the Form then the Number, and a paper whose weight has come to rest on its falsification register has misplaced its own foundation. **Two criteria, three at the ceiling.** Fewer and tighter beats more and looser at every count. This corrects the earlier reading of Section 7 in two places at once: the count comes down from three-to-five, and the section stops being a co-anchor of body length, which §I.B now states.

**Where the vulnerability comes from, named exactly, because the naming is the whole rule.** Not from the geometry, which is sealed. From the intake. A falsifier is built by taking something as true and predicting what follows from it, and the something taken as true is almost never derived by the paper. It is borrowed from whatever the surrounding field currently holds. That borrowing is the entire exposure. When the borrowed thing moves, and fields move, the criterion built on it fires against a paper that never depended on it, **and the pressure lands on the architecture rather than on the import.** The genealogy screen therefore runs on the falsifier's own terms and not on the paper's, because a falsifier is the one part of a paper whose intake nobody screens.

**The recorded misfire, and it is why this section exists rather than a preference.** The IPG and MOND family was carried as an orthodoxy import across the physical block of the register of record. Four falsification clauses were built on it. When the import retired under `APEX-PSP-PHYSICAL-SEED-01`, all four were re-typed on the same day, P-03's framing, P-06's framing, P-07's mechanism retired to the substrate hole with the prediction carried at reduced anchor, and P-21's framing retired entire. One coordinate, `sPSP-137`, was hard-deleted by named override, and the mechanism recorded against it was an anchor loop, SE-308 against P-21, a claim anchoring on its own prediction. Not one of those clauses had bad arithmetic. Each rested on a borrowed premise, and the premise is what broke.

**The standard, lifted from the Physical Seed rather than invented.** Its §VI clause 5 states it, one falsifier running against the author, and the gravity seed is the model to copy: `w(z) ≥ −1`, the phantom divide never crossed. Read the typing that travels with it. A **structural-necessity prediction forced by the tension reading**, so the geometry entails it. **Seed-native**, so nothing borrowed carries it. Binding **on two named conditions**, the tension being a real fluid and the null energy condition holding for it, so a reader can tell a miss from a hit. Kill-switch at one named coordinate. Danger running against the author, reported without softening. Forced, native, conditioned, aimed.

**The five tests, run in this order, before a criterion is drafted into Section 7.**

{. Necessity, first and hardest .} Does the paper's own geometry force this prediction? If Section 6's structure entails it, the criterion is **FORCED** and is admissible. If it rests on anything the paper takes as true and did not derive, it is an **IMPORT**, and an import is never printed as a headline criterion. It is re-derived until the geometry carries it, or dropped, or carried beneath the register as a named tripwire at its honest grade, which is the third option and the one to reach for when the import is genuinely informative to a reader.

{. Genealogy, on the falsifier itself .} Every load-bearing term of the criterion carries its origin and its drift-distance, on original definitions, exactly as the paper's own terms do. Name every import it rests on. Then apply the Carrying Law to the criterion: **a falsifier caps at the weakest import it borrowed and can never carry the paper's verdict further than the thing it borrowed.** A criterion resting on a consensus-mediated row carries nothing, and printing it as though it did is inflation running in the one direction nobody checks.

{. Blast radius .} Simulate the firing. If the event occurred tomorrow, what falls? The answer is written into the criterion, naming **one claim of this paper** and nothing above it. The register-of-record idiom is the model, `→ <named claim> conditional anchor compromised`, never the framework falsified. **Then check what else falls.** If the simulated firing reaches a sealed coordinate the paper did not author, the criterion is mis-aimed and is cut before drafting. A paper may put its own conditional anchors at risk. It may not volunteer the codex's.

{. The anchor loop .} The criterion must not anchor on a claim that anchors on the criterion. Walk the chain both ways. A loop is barred outright rather than repaired, because a loop is not a weak falsifier but a circle wearing one. Cut it at the claim side and re-derive the prediction from the geometry, exactly as the P-21 repair did.

{. Conditions and sharpness .} The criterion states the observable event that fires it, sharply enough that a reader knows what would count, and the conditions under which it binds. An unconditioned criterion misfires on the first reading that was never in its scope, and the misfire is charged to the paper rather than to the reader who found it.

**Two clarifications, because two disciplines would otherwise collide.**

Cutting a falsification criterion **is never a concession and needs no mechanism**, whether at genesis here or at upgrade under Module VI. A falsifier is not a claim. It is an invitation, and cutting a weak invitation strengthens a paper by narrowing what it volunteered. Module VI's Concession Gate does not fire on it. Without this clause the claim law and this gate would deadlock over exactly the tightening the order asks for.

A strong form and a weak form may travel together, and the sealed-halt card is the model: the strong form ties to **the claim the criterion actually makes**, and the weak form sits beneath it as the implementation tripwire. That pairing counts as one criterion against the ceiling and not two, because the weak form tests the implementation while the strong form tests the claim.

**What survives from the earlier reading, unchanged and now load-bearing.** Section 7's existing arithmetic discipline is already the right discipline and is kept whole: a quantified threshold expressed as a dimensionless ratio, a scaling exponent, a phase-transition point, or an absolute significance level; **no arbitrary human-fitted numerical constants**; every threshold parameter-free or derived from a single topological invariant; real instruments named at their resolution; a null hypothesis stated at the same precision as the confirmation criterion. A parameter-free threshold is what a forced criterion looks like from the arithmetic side, so the two halves of the rule were always the same rule read twice, and the gate above supplies the half that was missing.

**Where this gate binds.** Every Forge-mode paper at the mid-tier and fully-public tiers, at Step 3 of the pipeline, before Section 7 is drafted. Module VI inherits it at upgrade, where `automated-paper-upgrade` §2.7a mechanizes the same five tests in `spine_lock.py falsifier` and faults an unforced headline at verify. A paper forged under this gate arrives at that verify already passing, which is the point of seating it here.
...
## II.C · BOUNDARY TWO · THE SEATED CODEX COORDINATE

**Module II runs at two boundaries and not one.** Boundary one is the document deliverable, governed by II.A. Boundary two is the seating of a coordinate into a register of record, and it fires on every seating without a trigger phrase and without exception, exactly as Module IX does, the two composing at the same point: Module IX filters the register, this filters the archaeology.

**What a card body carries.** Standing law, and nothing about how the law was reached. A coordinate is not a report of an investigation. It is a register entry that a later session reads as current, and a body carrying *this was wrong and was corrected* forces that reader to adjudicate a history instead of reading a law. The nine steps of II.A apply unchanged: write the discipline directly, never write how it became, single voice, no meta-commentary about evolution.

**Where the correction goes, because it is never destroyed.** It has two homes already built for it and neither is the card. **The audit ledger**, where an erratum is filed as its own row with its identifier, its mechanism, and a pointer to the faulty ancestor, the census keeping the faulty row beside its correction with nothing retyped in place. And **the codex changelog stanza**, where an edition records what moved and why. A correction absent from a card body is therefore not a correction hidden; it is a correction filed where a reader looking for history will find it and a reader looking for law will not trip over it.

**The one exempt line.** A card's Source or Provenance line may state in a single sentence that a draft was superseded before seating, since that is provenance rather than archaeology. It carries no mechanism, no self-assessment, and no enumeration of errors. A narrative there is a body in disguise and is struck.

**The unseated draft owes no ghost.** Codex hygiene requires a two-line ghost with a recovery pointer whenever a seated entry is superseded. A draft that was never seated was never in the register, so nothing is superseded and no ghost is owed. The prior draft is parked, not ghosted, and the distinction matters because a ghost for a never-seated entry is a phantom row that the index-parity check will read as a fault.

**Gate 1 extends to card bodies.** Before any seating, sweep the card for the identifier prefix used by auditor errata, for version markers, and for first-person revision constructions: *I was wrong*, *my error*, *correction to my*, *previously I*, *earlier I stated*. Any hit is rewritten as native standing law or routed to the ledger. The sweep is mechanical and its result is recorded in the working log, never in the card.

**The scope of the strip, stated once so it is not over-read.** This governs deliverables and seated coordinates. It does not govern the session record, the audit ledger, the changelog, or this file, all of which are histories by function and carry their evolution in full. Stripping history from a document whose job is to hold history is the same error running the other way.
...
| author_name | architect block | Author name line, carrying the academic credential on every scholarly edition per the credential law below. |
...
**The credential law.** On every scholarly edition, Apex Pristine, Journal, Math Journal, Textbook, and Blog, the author line carries the academic credential as part of the name entity and not as an optional addition: `author_name: Mohammad F. Islam, PhD` in the Apex front matter, `author_line: Mohammad F. Islam, PhD^1^` in the Journal and Math Journal front matter with the affiliation marker after the credential, and the same form in a Blog byline. A byline without the credential on a scholarly edition is a Gate 1 failure and is repaired before any build. The credential is absent by design at the devotional register: the Scripture edition and every artifact under `REGISTER:DEVOTIONAL` carry the humble-servant line of III·S.2, since that register is witness and not authority, and a credential there is a register error in the other direction. A default shown in one edition's exemplar binds no other edition; this clause binds them all.
...
/* =====================================================================
   ...
   ===================================================================== */
:root{
  --serif:'EBGaramond','EB Garamond','Noto Sans Math','Liberation Serif','Noto Serif','DejaVu Serif',serif;
...
:root{
  --serif:'EBGaramond','EB Garamond','Noto Sans Math','Liberation Serif','Noto Serif','DejaVu Serif','FreeSerif',serif;
...
/* ===================================================================
   ...
   =================================================================== */
:root{
  --serif:'EBGaramond','EB Garamond','Noto Sans Math','Liberation Serif','Noto Serif',serif;
...
# MODULE III·E · THE ENCYCLOPEDIA EDITION

The reference register. Module III renders the Apex Pristine single-column treatise. Module III·J renders the Professional Journal Paper, two columns and a masthead. Module III·M renders its mathematical descendant. Module III·S renders Scripture, verse by verse. Modules V and V·D render the Textbook editions. The Encyclopedia edition is the sixth document renderer and it is the only one whose unit is not a section but an entry, filed alphabetically, addressed by a guide word, and readable from any page inward. Selected by the token `[EDITION:ENCYCLOPEDIA]`, front-matter `edition: encyclopedia`, or the triggers encyclopedia, encyclopaedia, reference work, alphabetic entries, dictionary pdf, and lexicon volume. It transforms any corpus into the reference register and co-fires with none of the other document renderers.

## III·E.0 · The Design Read

An alphabetic reference volume sets a page no other edition sets, and seven elements carry it. First, the type is a single transitional serif throughout at a small measure, roughly nine and a half points on a tight leading, justified and hyphenated, with no bold anywhere in the body; emphasis is carried by italic, small capitals, and size alone, because a page that must stay legible across six hundred pages stays legible by never raising its voice. Second, the running furniture is a guide word rather than a title: the head band carries the name of the entry in progress at the foot of the page in letterspaced small capitals, beside the folio, and the two are not flush to the margins but each centred over its own column, the guide word taking the outer column and the folio the inner on both verso and recto, so the folio hugs the gutter and the guide word sits where a thumb finds it. Third, each letter opens on its own page with a single large capital, centred, sunk about fifteen percent down the page and closed by a short accent bar, the first entries beginning below it on the same page, the opener carrying no running furniture. Fourth, the body is two justified columns at a wide gutter and it never balances, because a reference work is a continuous run of entries and balancing would strand space at every letter break. Fifth, the entry head is flush left, roman, half again the body size, with generous space above and none of the weight a section head carries, since an entry is announced rather than shouted. Sixth, the apparatus is dense and abbreviated: `(q.v.)` for a term that has its own entry, small capitals after see for a delegation, a small-cap sigil for scripture citation, italic short titles expanded once in the front matter, an italic signature closing every article, and a bibliography set two points down with a hanging indent and italic Primary and Secondary runs. Seventh, the volume is a volume, with full front matter repeated in each and a final apparatus volume carrying the author list, the article list, and the indices. The aesthetic is density and quiet, the accent touching only the letter bar, the guide word, the see-references, the box top rule, and the table caption bar.

## III·E.1 · The Locked Contract

Serif throughout, EB Garamond, body at 9.5 points on a 1.235 measure, justified and hyphenated, first-line indent of eleven points, and no indent on the paragraph opening an entry. Page geometry is royal octavo, 494 by 736 points, margins 82 by 64 by 92 by 64, with a two-column body at 168 points per column and a 30-point gutter and no column rule. The head band sits at 35 points with forty-seven points clear above the text block. There is no foot furniture; the folio rides the head band opposite the guide word. Type sizes are fixed: entry head 12.6, sub-head 10 italic, body 9.5, running head 7.6 small caps, bibliography 8.1, box 8.6, table 8.2, index 8.2, letter glyph 46. The universal locks hold without exception: no LaTeX and all mathematics in Unicode, no em-dashes, the honorific ﷺ as one glyph, no emoji, honorifics preserved through every gloss and index row, and no box, figure, or table fragmenting except where a long bibliography or index is explicitly allowed to flow.

Font provisioning is a hard pre-flight check for this edition: EB Garamond for the serif, Amiri for Arabic runs, Noto Serif Bengali for Bengali runs, and Noto Sans Math for the ⟀ seal glyph. Variable faces are instanced through `fontTools.varLib.instancer` before WeasyPrint consumes them, and `fc-cache -f ~/.fonts` runs after placement. A build that renders ⟀ as tofu has skipped the math face and is a defect, not a render.

## III·E.2 · The Guide Word Law

The running head is the edition's navigation and it obeys one rule with three consequences. The guide word is the name of the last entry to begin on or before the current page, which is to say the entry in progress at the page foot, implemented as `string-set: guide content()` on the entry head and `string(guide, last)` in the margin box, which resolves to exactly that entry with no authoring intervention; an author never types a running head. Three consequences bind. A page carrying only the tail of an entry that began earlier still shows that entry's name, which is correct, since a reader thumbing to that page is inside that entry. A letter opener shows no guide word at all, because the large capital is the navigation on that page and a small-cap repetition beneath it is noise. And a redirect stub does not set a guide word, because a one-line pointer is not an entry a reader is inside of; stubs are authored as paragraphs rather than heads precisely so the string does not move. The outer-and-inner placement is set by `@page body:left` and `@page body:right` and must be carried on both, or the alternation fails silently and reads as a random head.

## III·E.3 · The Entry and Its Eight Slots

The unit of the edition. Order is fixed; slots one and three are mandatory and the rest fire where the entry warrants. Slot one is the head, `# Head (vocalisation)`, flush left and roman, setting the guide word. Slot two is the vocalisation, an italic parenthetical carrying the source-language form behind the gloss. Slot three is the definition, the first paragraph, unindented, defining before it discusses. Slot four is the body under `## sub` and `### run-in` heads, justified and indented. Slot five is the see-also apparatus, `(q.v.)` and small-capital see-references woven inline and never set as a separate block. Slot six is the signature, `@ Author Name`, italic and flush left, closing the article. Slot seven is the bibliography at 8.1 points with a hanging indent and Primary and Secondary runs. Slot eight is the seal, `!seal ⟀ GOL`, the one shown verdict where the entry carries one.

The redirect stub is the ninth form and it is a complete entry in one line: `> Barrier -> BARZAKH` renders as Barrier, see barzakh. Redirect stubs are what make a reference work findable, and under-stubbing is the standard failure of a first edition; budget roughly one stub for every two substantive entries.

The `(q.v.)` and see-reference distinction is load-bearing and must not be collapsed. A `(q.v.)` is a pointer, meaning the word just read is itself an entry. A small-capital see-reference is a delegation, meaning this matter is not treated here but there. The first moves a reader laterally and the second stops an entry from restating another; an edition that collapses them becomes a set of overlapping essays with an index attached.

The seal is the one shown verdict and the silence law of the scripture instrument governs everything else. The verification apparatus runs in full and stays internal: warrant strings, verdict glyphs other than the seal, PSP identifiers, and mode tags never reach the page. Where an entry's reading does not close, the entry says so in plain words and shows no seal.

## III·E.4 · The Letter Opener

Each letter opens on its own page. The opener is a `column-span:all` element carrying the capital at 46 points, centred, at 110 points from the head band, closed by a 44-point accent bar, with 96 points of sink below it before the columns resume, and entries begin on the same page beneath it. Two rules bind. The opener breaks to a new page and never sits mid-page, because a letter break mid-column reads as an accident. And the opener suppresses the running head, which follows automatically from the guide-word law since no entry head has yet been passed on that page. Where a volume's range splits a letter, as P–Sh against Si–Z, the second volume's opener carries the letter with the range beneath it rather than the bare capital.

## III·E.5 · Boxes, Tables, and Figures

Carried from the Journal contract at reference scale. The box is a tinted panel with an accent top rule and a small-caps sans header at 8.6 points, used where a fence or a caveat must be visible rather than buried in running prose. The table is a captioned object, an accent caption bar above a ruled table with a tinted header row and a zebra body at 8.2 points. The figure is framed with a small-caps accent label. All three default to column width and flow inside one column, which packs the dense page; spanning is opt-in through `:::box*`, `Table*:`, and `!*[`, and is reserved for a table genuinely too wide to read in one column, since a spanning element closes both columns and strands the band above it. Boxes flow and break across the column boundary, the panel borderless so a broken panel reads as a clean continuation and never as an open cut edge. Every claim-bearing box and table carries its warrant tier exactly as the codex establishes it; the faithful-map law governs and a box asserts no more than the source proves.

## III·E.6 · Front Matter and the Apparatus Volume

Front matter runs on roman folios with the division name as the running head, in fixed order: half-title, editorial board, title page, copyright, preface, contents, abbreviations, authors of articles, short titles, transliteration. The full front matter repeats in every text volume, because a volume must be usable alone on a desk. Two divisions do load-bearing work. Short titles is the citation engine, since every bibliography in the body cites by short form and the expansion lives once, here, which is what keeps bibliographies at two lines instead of six; build it before the first entry is written. Authors of articles is the reverse index of the corpus and makes the volume auditable, since an entry with no author there is an orphan and an author with no entries is a ghost.

The apparatus volume is a volume rather than an appendix and carries five divisions: author list, article list, index of proper names, index of source-language terms, and index of citations. The citation index is what justifies the apparatus volume, since a reader holding a passage and wanting every entry that treats it has no other route in; building it requires that every citation in the body be tagged at authoring time, which is why the citation mark is a distinct inline form rather than plain text. Index rows carry a volume sigil in small-cap roman before the page numbers, a multi-volume index without a volume marker being unusable.

Budget the apparatus at roughly a fifth of total extent. Measured against a mature six-volume reference set, the index volume runs 865 pages against 3,088 pages of text, twenty-two percent of the whole. That ratio is not decoration, it is what makes the other seventy-eight percent reachable, and it is the number a first edition most often fails to plan for.

## III·E.7 · The Filing Law

An encyclopedia is filed by string, not by initial, and the sort key is fixed before the first entry is written because re-filing a finished corpus is the one edit that cannot be automated safely. The key strips a leading article (`al-`, `el-`, `the`, `a`, `an`), decomposes to NFKD and drops combining marks, drops hamza and the modifier letters, drops all non-alphanumerics, and lowercases, so al-Baqara files at B, ʿĀd files at A, and Ṣāliḥ files at S. The engine holds the corpus to this key and emits a warning naming the entry before which the order breaks; that warning is a source defect repaired before delivery and never a render accepted out of order. A volume range may split a letter, as P–Sh against Si–Z, since the key is a string.

## III·E.8 · Entry Tiers and Scope Drift

Two planning laws, both measured rather than asserted. Entry length distributes with a violent right skew: against a mature reference set the median entry runs two to three pages while the longest runs to thirty-four. A flat words-per-entry plan cannot produce that shape, so tiers are declared before drafting, a stub against a standard entry against a major article, and the tier is fixed at commission rather than discovered at draft, or every entry drifts toward the middle and the work loses its topography.

Scope drift is the second and it is visible in any multi-volume set built over years: the same measured corpus ran 178 entries at 3.1 pages each in its first volume and 119 at 5.1 pages in its fourth, entries growing longer and fewer as the editors went on. The fix is to fix mean length per tier at volume one and audit each volume against it, never to discover the drift at volume four.

## III·E.9 · Authoring Conventions

Front matter sets the volume: `edition: encyclopedia`, then `title`, `volume`, `range`, and `accent`. Accent is one of copper, crimson, slate, navy, forest, or plum; copper is the default and reads as a scholarly reference volume where crimson reads as a journal.

The source runs as ordinary prose inside directive blocks. `:::plate` opens a full-page display with no running furniture, its parts set by `%key value` lines. `:::front` opens roman folios with a division-name running head, and `:::section NAME` opens a front-matter division under a ruled small-cap head. `:::corpus` opens the body, arabic folios restarting at one, two columns, guide-word head. `:::letter A` sets a letter opener. `# Head (vocalisation)` sets an entry head, a parenthetical carrying diacritics rendering as vocalisation. `> Variant -> TARGET` sets a redirect stub. `## Sub` sets an italic sub-head and `### Run-in` an italic run-in head flowing into its paragraph. `@ Name` sets the signature. A `Bibliography` line opens the hanging block until a blank line, with `Primary:`, `Secondary:`, `Sources:`, and `Studies:` runs. `!seal ⟀ GOL` sets the shown verdict. `:::box TITLE` and `:::box*` set the panel, and `Table: caption` followed by pipe rows sets a captioned table. `:::back` opens the apparatus volume and `:::bmsection NAME` its divisions; `:::idx` with `$ A`, `. row`, and `.. subrow` sets an index block at two depths, `:::artlist` with `. row` sets the article list, `:::reflist` sets the two-column hanging list used for abbreviations and short titles, `:::translit` sets the three-column glyph table, and `:::toc` with `| label | page` sets contents rows.

Inline marks: `~short title~` sets an italic short-title citation, `[[see:TERM]]` and `[[TERM]]` set a small-caps see-reference, `(q.v.)` is picked up literally, `Q 2:130` is picked up by pattern and set with a small-cap sigil, `{{ar:...}}` and `{{bn:...}}` open script runs, and `*italic*` and `^sup^` behave as usual. Soft-wrapped source lines join into one paragraph and a blank line ends it.

## III·E.10 · The Encyclopedia Stylesheet

The complete validated stylesheet, self-contained, compiling under WeasyPrint as written. Save as `encyclopedia.css` beside the source; the face block of Module V provisions the serif and the sans, with the system fallback chain applying if it is skipped. Four named page contexts carry the volume: `front` for roman folios, `body` for the guide-word alternation, `back` for the indices, and `plate` for display pages with no furniture.

```css
/* ===================================================================
   ENCYCLOPEDIA  ·  Alphabetic Reference Edition stylesheet
   (WeasyPrint-validated)
   Brill royal-octavo page 494x736pt   two columns 168pt / 30pt gutter
   Dictionary running head: guide word over the OUTER column, folio over
   the INNER column.  Letter-opener pages.  Flush-left entry heads.
   Small-caps see-references.  Hanging-indent two-run bibliography.
   Italic signature.  Journal box / table apparatus carried in.
   =================================================================== */

:root{
  --serif:'EBGaramond','EB Garamond','Noto Sans Math','Liberation Serif','Noto Serif','DejaVu Serif','FreeSerif',serif;
  --sans:'TBSans','Archivo','Liberation Sans','Noto Sans','DejaVu Sans','FreeSans',sans-serif;
  --arab:'Amiri','Scheherazade New','Noto Naskh Arabic','Noto Serif',serif;
  --beng:'Noto Serif Bengali','Noto Sans Bengali',serif;
  --ink:#1a1a1a; --soft:#3d3d3d; --faint:#6f6f6f; --rule:#c9c9c9;
  --accent:#B87333; --deep:#8a5523; --tint:#FAF3EA; --band:#EFE0CF;
}
.acc-copper {--accent:#B87333;--deep:#8a5523;--tint:#FAF3EA;--band:#EFE0CF;}
.acc-crimson{--accent:#A0182B;--deep:#74111F;--tint:#FBEDEF;--band:#F4DEE2;}
...
/* ================= PAGE GEOMETRY ================= */
/* Trim 494x736pt.  Text block 366x562.  Head band at 35pt.  Foot 92pt. */
@page{
  size:494pt 736pt;
  margin:82pt 64pt 92pt 64pt;
}

/* ---- BODY pages: dictionary running head, guide word OUTER ---- */
@page body:left{
  @top-left{ content:string(guide, last); font-family:var(--serif); font-size:7.6pt;
    font-variant:small-caps; letter-spacing:.09em; color:var(--deep);
    vertical-align:bottom; padding-bottom:12pt; }
  @top-right{ content:counter(page); font-family:var(--serif); font-size:8.4pt;
    color:var(--soft); vertical-align:bottom; padding-bottom:12pt; }
}
@page body:right{
  @top-left{ content:counter(page); font-family:var(--serif); font-size:8.4pt;
    color:var(--soft); vertical-align:bottom; padding-bottom:12pt; }
  @top-right{ content:string(guide, last); font-family:var(--serif); font-size:7.6pt;
    font-variant:small-caps; letter-spacing:.09em; color:var(--deep);
    vertical-align:bottom; padding-bottom:12pt; }
}
/* ---- FRONT matter: roman folios, section name as head ---- */
@page front{
  counter-reset:none;
  @top-left{ content:string(secname); font-family:var(--serif); font-size:7.6pt;
    font-variant:small-caps; letter-spacing:.09em; color:var(--deep);
    vertical-align:bottom; padding-bottom:12pt; }
  @top-right{ content:counter(page, lower-roman); font-family:var(--serif);
    font-size:8.4pt; color:var(--soft); vertical-align:bottom; padding-bottom:12pt; }
}
/* ---- BACK matter: index running heads ---- */
@page back:left{
  @top-left{ content:string(secname); font-family:var(--serif); font-size:7.6pt;
    font-variant:small-caps; letter-spacing:.09em; color:var(--deep);
    vertical-align:bottom; padding-bottom:12pt; }
...
@page back:right{
  @top-left{ content:counter(page); font-family:var(--serif); font-size:8.4pt;
    color:var(--soft); vertical-align:bottom; padding-bottom:12pt; }
  @top-right{ content:string(secname); font-family:var(--serif); font-size:7.6pt;
    font-variant:small-caps; letter-spacing:.09em; color:var(--deep);
    vertical-align:bottom; padding-bottom:12pt; }
}
/* ---- Display pages carry no furniture ---- */
@page plate{ @top-left{content:none} @top-right{content:none} }

/* ================= BASE TYPE ================= */
html{ font-family:var(--serif); font-size:9.5pt; color:var(--ink); line-height:1.235; }
body{ margin:0; }
p{ margin:0; text-indent:11pt; text-align:justify; hyphens:auto; orphans:2; widows:2; }
p.noind, p:first-of-type{ text-indent:0; }
sup{ font-size:.66em; line-height:0; vertical-align:super; }
em,i{ font-style:italic; } strong,b{ font-weight:700; }
a{ color:inherit; text-decoration:none; }
.ar{ font-family:var(--arab); font-size:.98em; direction:rtl; unicode-bidi:isolate;
     line-height:inherit; vertical-align:baseline; }
/* display Arabic: own paragraph, own leading, vocalisation licensed here only */
p:has(> .ar:only-child){ text-indent:0; text-align:center; line-height:2.0;
     margin:.4em 0 .45em; }
.bn{ font-family:var(--beng); font-size:.95em; }

/* ================= FRONT MATTER ================= */
.front{ page:front; }
.front .fm{ break-before:page; }
.plate{ page:plate; break-before:page; text-align:center; padding-top:150pt; }
.plate .brand{ font-family:var(--serif); font-size:9pt; font-variant:small-caps;
  letter-spacing:.34em; color:var(--accent); margin:0 0 40pt; }
.plate .wtitle{ font-family:var(--serif); font-size:29pt; line-height:1.16; margin:0 0 6pt; }
.plate .wsub{ font-family:var(--serif); font-style:italic; font-size:13.5pt;
  color:var(--accent); margin:0 0 34pt; }
.plate .vol{ font-family:var(--serif); font-size:11pt; font-variant:small-caps;
  letter-spacing:.2em; margin:0 0 3pt; }
.plate .range{ font-family:var(--serif); font-size:17pt; letter-spacing:.16em; margin:0 0 46pt; }
.plate .ed{ font-size:10pt; margin:0 0 40pt; }
.plate .imp{ font-size:9pt; color:var(--soft); }
.plate .rule{ width:78pt; height:1.6pt; background:var(--accent); margin:0 auto 34pt; }

.fmhead{ font-family:var(--serif); font-size:14pt; font-variant:small-caps;
  letter-spacing:.13em; color:var(--deep); text-align:center; margin:0 0 18pt;
  padding-bottom:6pt; border-bottom:.6pt solid var(--rule); string-set:secname content(); }
.front .fm p{ text-indent:13pt; }
.front .fm p.noind{ text-indent:0; }

/* two-column reference lists in the front matter (abbreviations, short titles) */
.reflist{ column-count:2; column-gap:22pt; font-size:8.1pt; line-height:1.22; }
.reflist p{ text-indent:0; padding-left:9pt; margin:0 0 3.2pt; text-align:left;
  hyphens:none; break-inside:avoid; }
.reflist p > span.k{ margin-left:-9pt; }
.reflist .k{ font-variant:small-caps; letter-spacing:.03em; }

/* transliteration table */
.translit{ column-count:3; column-gap:20pt; font-size:8.4pt; line-height:1.5; }
.translit .row{ break-inside:avoid; display:flex; justify-content:space-between;
  border-bottom:.35pt dotted var(--rule); padding:.6pt 0; }
.translit .row .g{ font-family:var(--arab); font-size:1.25em; }
.translit .row .t{ font-style:italic; color:var(--deep); }

/* contents list */
.toc{ font-size:9.4pt; }
.toc .row{ display:flex; justify-content:space-between; border-bottom:.35pt dotted var(--rule);
  padding:3.4pt 0; break-inside:avoid; }
.toc .row .lbl{ font-variant:small-caps; letter-spacing:.05em; }
.toc .row .pg{ color:var(--faint); }

/* ================= BODY · THE ALPHABET ================= */
.corpus{ page:body; counter-reset:page 1; }
.entries{ column-count:2; column-gap:30pt; column-fill:auto; }

/* letter opener: spans both columns, sinks the text, no running head */
.letter{ column-span:all; break-before:page; text-align:center;
  padding:34pt 0 96pt; }
.letter .glyph{ font-family:var(--serif); font-size:46pt; line-height:1;
  color:var(--ink); letter-spacing:.02em; }
.letter .bar{ width:44pt; height:1.2pt; background:var(--accent); margin:16pt auto 0; }

/* entry head: flush left, roman, generous space above, sets the guide word */
h2.entry{ font-family:var(--serif); font-weight:400; font-size:12.6pt; line-height:1.14;
  color:var(--ink); margin:15pt 0 6pt; break-after:avoid;
  string-set:guide content(); }
h2.entry:first-child{ margin-top:0; }
h2.entry .vocal{ font-style:italic; font-size:.82em; color:var(--deep); }

/* subdivision heads inside a long entry */
h3.sub{ font-family:var(--serif); font-weight:400; font-style:italic; font-size:10pt;
  color:var(--deep); margin:8pt 0 3pt; break-after:avoid; }
h4.runin{ display:inline; font-family:var(--serif); font-weight:400; font-style:italic;
  font-size:1em; color:var(--ink); }
h4.runin + p{ display:inline; text-indent:0; }
.lead{ font-style:italic; }

/* cross-reference apparatus */
.qv{ font-style:normal; }                       /* (q.v.) */
.see{ font-variant:small-caps; letter-spacing:.04em; color:var(--deep); }
.cit{ font-variant:small-caps; letter-spacing:.03em; }   /* Q 2:130 */
.st{ font-style:italic; }                       /* short titles */

/* redirect stub: "ABRAHAM, see ..." */
p.redirect{ text-indent:0; margin:15pt 0 0; }
p.redirect .rhead{ font-size:12.6pt; }
p.redirect .rto{ font-variant:small-caps; letter-spacing:.04em; color:var(--deep); }

/* signature */
p.sig{ text-indent:0; text-align:left; font-style:italic; font-size:9.5pt;
  margin:5pt 0 2pt; break-before:avoid; }

/* bibliography: small, hanging indent, Primary / Secondary runs */
.bib{ font-size:8.1pt; line-height:1.2; margin:3pt 0 2pt; break-inside:auto; }
.bib .bhead{ font-family:var(--serif); font-style:normal; font-variant:small-caps;
  letter-spacing:.06em; color:var(--deep); display:block; margin:0 0 1pt; }
.bib p{ text-indent:0; padding-left:9pt; margin:0 0 2pt; text-align:left; hyphens:auto; }
.bib p > .run{ margin-left:-9pt; font-style:italic; color:var(--deep); }

/* see also, the navigation tail: set apart above the signature, never a heading */
p.seealso{ text-indent:0; text-align:left; font-size:8.6pt; line-height:1.24;
           margin:4.2pt 0 0; padding-top:2.4pt; border-top:.4pt solid var(--rule);
           color:var(--ink); }
p.seealso .salab{ font-variant:small-caps; letter-spacing:.05em; color:var(--deep); }
p.seealso .seed{ font-variant:small-caps; letter-spacing:.03em; }

/* seal line, the one shown verdict */
p.seal{ text-indent:0; text-align:left; font-size:9pt; color:var(--deep);
  letter-spacing:.05em; margin:4pt 0 0; break-before:avoid; }

/* ================= APPARATUS carried from the Journal ================= */
.box{ background:var(--tint); border-top:1.8pt solid var(--accent);
  padding:6pt 8pt 6pt; margin:7pt 0 8pt; font-size:8.6pt; line-height:1.26; }
.box .bxh{ font-family:var(--sans); font-weight:700; font-size:7.4pt; color:var(--deep);
  text-transform:uppercase; letter-spacing:.07em; margin:0 0 3.5pt; }
.box p{ text-indent:0; margin:0 0 3.5pt; }
.box.span{ column-span:all; }

table{ border-collapse:collapse; width:100%; font-size:8.2pt; line-height:1.2;
  margin:2pt 0 8pt; }
.tcap{ font-family:var(--sans); font-weight:700; font-size:7.2pt; color:#fff;
  background:var(--accent); text-transform:uppercase; letter-spacing:.06em;
  padding:2.6pt 6pt; margin:7pt 0 0; break-after:avoid; }
th{ background:var(--band); font-weight:700; text-align:left; padding:2.6pt 4pt;
  border-bottom:.7pt solid var(--accent); }
td{ padding:2.4pt 4pt; border-bottom:.35pt solid var(--rule); vertical-align:top; }
tbody tr:nth-child(even) td{ background:#fafafa; }
figure{ margin:5pt 0 8pt; } figure img{ width:100%; }
figcaption{ font-size:7.6pt; color:var(--soft); line-height:1.22; margin-top:2.5pt; }
figcaption .lab{ font-variant:small-caps; letter-spacing:.05em; color:var(--accent); }
.span{ column-span:all; }

/* ================= BACK MATTER ================= */
.back{ page:back; }
.back .bm{ break-before:page; }
.idx{ column-count:2; column-gap:26pt; font-size:8.2pt; line-height:1.24; }
.idx .il{ break-inside:avoid; padding-left:10pt; margin:0 0 1.6pt; text-indent:-10pt;
  text-align:left; hyphens:none; }
.idx .il.d2{ padding-left:20pt; text-indent:-10pt; }
.idx .ltr{ column-span:none; font-size:10pt; font-variant:small-caps; letter-spacing:.12em;
  color:var(--accent); margin:7pt 0 3pt; break-after:avoid; break-inside:avoid; }
.idx .il .f{ color:var(--faint); }
.idx .il .v{ font-variant:small-caps; letter-spacing:.03em; color:var(--deep); }
.artlist{ column-count:2; column-gap:26pt; font-size:8.4pt; line-height:1.26; }
.artlist .al{ break-inside:avoid; padding-left:10pt; text-indent:-10pt; margin:0 0 2pt;
  text-align:left; hyphens:none; }
.artlist .al .au{ font-style:italic; color:var(--soft); }
.colophon{ break-before:page; text-align:center; padding-top:210pt; font-size:8.6pt;
  color:var(--soft); page:plate; }
.colophon .rule{ width:60pt; height:1.2pt; background:var(--accent); margin:0 auto 18pt; }
```

## III·E.11 · The Encyclopedia Build Engine

The entry-aware engine. It parses the front matter and the directive blocks, renders the plate and the front-matter divisions, opens the corpus in two columns, emits the letter openers, sets the entry heads with their `string-set` guide words, joins soft-wrapped lines into paragraphs, closes the bibliography on a blank line, renders the boxes, captioned tables, index blocks, and article lists, and computes the filing key for every head. Save as `build_encyclopedia.py`. Build with `python3 build_encyclopedia.py source.md /mnt/user-data/outputs/{slug}.pdf`, the base URL the source directory, then inspect the render and confirm seven things: the letter opener sinks and carries no running head; the guide word sits over the outer column and the folio over the inner, alternating correctly across a spread; the entry heads are roman and unbolded; the run-in heads flow into their paragraphs; the redirect stubs sit in filing order; the bibliographies hang at 8.1 points with italic runs; and the ⟀ seal renders as a glyph rather than tofu. Present the PDF and keep the Markdown source as master.

The engine emits two warnings to standard error and both are source defects never accepted as renders: an alphabetic order break, naming the entry that breaks it, and an unsortable head, naming a head whose filing key resolves empty.

```python
#!/usr/bin/env python3
"""
ENCYCLOPEDIA build engine (Alphabetic Reference Edition).
Compiles one Markdown source to a two-column royal-octavo reference PDF
via WeasyPrint. Journal apparatus + dictionary page furniture.

Usage:  python3 build_encyclopedia.py source.md [out.pdf]
Source dir must contain encyclopedia.css (and optionally fonts/).
"""
import sys, re, html, pathlib, unicodedata
...
def sortkey(s):
    """Alphabetic filing key: strip diacritics, hamza, articles, case."""
    s = re.sub(r'^(al-|el-|the |a |an )', '', s.strip(), flags=re.I)
...
def inline(t):
    t = esc(t)
    # protected constructs first
    t = re.sub(r'\{\{ar:(.+?)\}\}', r'<span class="ar">\1</span>', t)
    t = re.sub(r'\{\{bn:(.+?)\}\}', r'<span class="bn">\1</span>', t)
    # see-reference -> small caps          [[see:IDOLATRY]]
    t = re.sub(r'\[\[see:(.+?)\]\]', lambda m: '<span class="see">%s</span>' % m.group(1).lower(), t)
    # bare cross-reference                 [[FOREIGN VOCABULARY]]
    t = re.sub(r'\[\[(.+?)\]\]', lambda m: '<span class="see">%s</span>' % m.group(1).lower(), t)
    # q.v. marker                          (q.v.)
    t = t.replace('(q.v.)', '<span class="qv">(q.v.)</span>')
    # scripture citation                   Q 2:130   Q 105
    t = re.sub(r'\bQ\s(\d+[:\d\-,;\s]*\d*)', r'<span class="cit">q&#160;\1</span>', t)
    # short title (italic)                 ~Tabari, Tafsir~
...
# ----------------------------------------------------------------- parser
def parse(src):
    # hard rule R: scribe-facing HTML comments are internal and must never
    # surface in a rendered deliverable; stripped before any parsing occurs
    src = re.sub(r'<!--.*?-->', '', src, flags=re.S)
    fm, body = {}, src
    if src.startswith('---'):
        end = src.index('\n---', 3)
        for ln in src[3:end].strip().split('\n'):
            if ':' in ln:
                k, v = ln.split(':', 1)
                fm[k.strip()] = v.strip()
        body = src[end + 4:]
...
def render_body(body):
    out, i = [], 0
    lines = body.split('\n')
    entries = []          # (head, sortkey) for the article list
    index_terms = []      # (term, entry) for the index
    n = len(lines)
    in_bib = False
    ...
    while i < n:
        ...
        if s.startswith(':::'):
            ...
            if tag.startswith('letter '):
                out.append('<div class="letter"><div class="glyph">%s</div>'
                           '<div class="bar"></div></div>' % esc(tag[7:].strip()))
            elif tag.startswith('plate'):
                out.append('<div class="plate">')
            elif tag.startswith('front'):
                out.append('<div class="front">')
            elif tag.startswith('corpus'):
                out.append('<div class="corpus"><div class="entries">')
            elif tag.startswith('back'):
                out.append('</div></div><div class="back">')
            elif tag.startswith('section '):
                out.append('<div class="fm"><div class="fmhead">%s</div>' % esc(tag[8:].strip()))
            elif tag.startswith('bmsection '):
                out.append('<div class="bm"><div class="fmhead">%s</div>' % esc(tag[10:].strip()))
            elif tag.startswith('box'):
                sp = ' span' if tag.startswith('box*') else ''
                ttl = tag.split(' ', 1)[1] if ' ' in tag else ''
                out.append('<div class="box%s"><div class="bxh">%s</div>' % (sp, esc(ttl)))
            elif tag in ('reflist', 'translit', 'toc', 'idx', 'artlist'):
        ...
        if s.startswith('# '):
            ...
            voc = ''
            m = re.match(r'^(.*?)\s*\((.+)\)$', head)
            if m and re.search(r'[\u0100-\u02ff\u0621-\u064a]', m.group(2)):
                head, voc = m.group(1), m.group(2)
            entries.append((head, sortkey(head)))
            h = '<h2 class="entry">%s' % inline(head)
            if voc:
                h += ' <span class="vocal">(%s)</span>' % inline(voc)
            out.append(h + '</h2>')
            i += 1
            continue
        ...
        if s.startswith('> '):
            ...
            if m:
                entries.append((m.group(1), sortkey(m.group(1))))
                out.append('<p class="redirect"><span class="rhead">%s</span>, see '
                           '<span class="rto">%s</span>.</p>'
                           % (inline(m.group(1)), esc(m.group(2)).lower()))
            i += 1
            continue

        if s.startswith('## '):
            close_bib(); out.append('<h3 class="sub">%s</h3>' % inline(s[3:])); i += 1; continue
        if s.startswith('### '):
            close_bib(); out.append('<h4 class="runin">%s.</h4>' % inline(s[4:])); i += 1; continue

        # ---------- signature ----------
        if s.startswith('@ '):
            close_bib(); out.append('<p class="sig">%s</p>' % inline(s[2:])); i += 1; continue

        # ---------- see also, the navigation tail ----------
        if s.startswith('!seealso '):
            close_bib()
            items = [x.strip() for x in s[9:].split('\u00b7') if x.strip()]
            seed, rest = (items[0], items[1:]) if items else ('', [])
            out.append('<p class="seealso"><span class="salab">See also</span> '
                       '<span class="seed">%s</span>%s</p>'
                       % (inline(seed),
                          (' &#183; ' + ' &#183; '.join(inline(x) for x in rest)) if rest else ''))
            i += 1; continue

        # ---------- seal ----------
        if s.startswith('!seal '):
            close_bib(); out.append('<p class="seal">%s</p>' % inline(s[6:])); i += 1; continue

        # ---------- bibliography ----------
        if s == 'Bibliography' or s.startswith('Bibliography'):
            close_bib()
            out.append('<div class="bib"><span class="bhead">Bibliography</span>')
            in_bib = True; i += 1; continue
        if in_bib and s:
            buf = [s]
            i += 1
            while i < n and lines[i].strip() and not lines[i].strip().startswith(
                    (':::', '#', '>', '@ ', '!seal', '!seealso', '| ', '. ', '$ ', '%', 'Table:')) \
                    and not re.match(r'^(Primary|Secondary|Sources|Studies):', lines[i].strip()):
                buf.append(lines[i].strip()); i += 1
            joined = ' '.join(buf)
            m = re.match(r'^(Primary|Secondary|Sources|Studies):\s*(.*)$', joined)
            if m:
                out.append('<p><span class="run">%s:</span> %s</p>'
                           % (m.group(1), inline(m.group(2))))
            else:
                out.append('<p>%s</p>' % inline(joined))
            continue
        if in_bib and not s:
            close_bib(); i += 1; continue
        ...
        if s.startswith('| '):
            ...
            if len(parts) >= 2:
                out.append('<div class="row"><span class="lbl">%s</span>'
                           ...
                           % (inline(parts[0]), esc(parts[1])))
            i += 1; continue
        if s.startswith('. ') or s.startswith('.. '):
            d = 'il d2' if s.startswith('.. ') else 'il'
            out.append('<div class="%s">%s</div>' % (d, inline(s.lstrip('. '))))
            i += 1; continue
        if s.startswith('$ '):
            out.append('<div class="ltr">%s</div>' % esc(s[2:]))
            i += 1; continue

        # ---------- table ----------
        if s.startswith('Table:'):
            close_bib()
            out.append('<div class="tcap">%s</div>' % esc(s[6:].strip()))
            i += 1
            rows = []
        ...
        if s.startswith('%'):
            k, _, v = s[1:].partition(' ')
            out.append('<div class="%s">%s</div>' % (k, inline(v)))
            i += 1; continue

        # ---------- paragraph (soft-wrapped lines join) ----------
        if s:
            close_bib()
            cls = ' class="noind"' if s.startswith('^^') else ''
...
# ----------------------------------------------------------------- shell
def build(srcpath, outpath):
    srcdir = pathlib.Path(srcpath).parent
    src = pathlib.Path(srcpath).read_text(encoding='utf-8')
    fm, body = parse(src)
    bodyhtml, entries = render_body(body)

    accent = fm.get('accent', 'copper')
    doc = f"""<!DOCTYPE html><html><head><meta charset="utf-8">
<title>{esc(fm.get('title','Encyclopedia'))}</title></head>
<body class="acc-{accent}">
{bodyhtml}
</body></html>"""
    (srcdir / '_encyclopedia.html').write_text(doc, encoding='utf-8')
    HTML(string=doc, base_url=str(srcdir)).write_pdf(
        outpath, stylesheets=[CSS(filename=str(srcdir / 'encyclopedia.css'))])

    print(f"built {outpath}")
    print(f"entries parsed: {len(entries)}")
    bad = [h for h, k in entries if not k]
    if bad:
        print(f"WARNING unsortable heads: {bad}", file=sys.stderr)
    order = [k for _, k in entries]
    if order != sorted(order):
        offs = [entries[i][0] for i in range(1, len(order)) if order[i] < order[i - 1]]
        print(f"WARNING alphabetic order break before: {offs}", file=sys.stderr)


if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("usage: build_encyclopedia.py source.md [out.pdf]"); sys.exit(1)
    out = sys.argv[2] if len(sys.argv) > 2 else str(pathlib.Path(sys.argv[1]).with_suffix('.pdf'))
    build(sys.argv[1], out)
```

## III·E.12 · Inheritance and Departure

Inherited from the Journal contract: the accent system and its six named accents, serif throughout, the WeasyPrint pipeline and the front-matter grammar, the tinted box with its accent top rule, the captioned table with its accent bar and zebra body, the figure label, the span rule and its asterisk form, the running furniture driven by `string-set`, and every universal lock. Inherited from the reference-work form: the royal-octavo page and its 168-and-30 column measure, the guide-word running head with its outer-and-inner inversion, the letter opener, the flush-left roman entry head, the `(q.v.)` and see-reference apparatus, the italic signature, the two-run hanging bibliography, the repeated front matter, and the five-division apparatus volume.

Four departures are declared rather than silent. An accent enters where the reference form is black throughout, and it touches only the letter bar, the guide word, the see-references, the box top, and the table bar. The seal line is added as the one shown verdict, since the reference form carries no verdict register. The box and the captioned table are imported, since a geometric reference work needs a place to put a fence and a place to put a comparison and burying either in running prose loses it. And the bibliography runs are extended with `Sources` and `Studies` for corpora where the primary-and-secondary split does not apply. Everything else, the page, the columns, the running-head inversion, the entry anatomy, the apparatus grammar, and the back-matter taxonomy, is held as measured.

## III·E.13 · The Front-Matter Description Cap

The skill's own `description` field is capped at 1024 characters by the loader, and every edition that adds a renderer grows it, because that field is where the routing triggers live. The cap is therefore an authoring constraint on this module and on every edition module after it: an edition contributes its selector and at most two or three natural-language triggers to the description, never a prose gloss of what the edition does. The gloss belongs in the module, which has no cap. A selector is never dropped to make room, since a missing selector costs a route while a missing alias costs nothing; where the field will not fit, redundant aliases are cut first. Measure the string before delivery, never after install, and compose to roughly nine hundred so the next edition has somewhere to land.

## III·E.14 · The Uniform-Grid Law and the Arabic Line Box

The dense two-column reference page earns its legibility from a uniform grid, and any element that opens the leading of one paragraph and not its neighbours destroys it. Vocalised Arabic is that element. Amiri with full tashkil is taller than the 9.5-on-1.235 body line box, so inline Arabic collides with the lines above and below it, and the obvious repair, opening the leading on any paragraph that carries Arabic, stops the collision by breaking the grid. That repair is barred. Two rules replace it.

**Inline Arabic is unvocalised and size-matched.** Set at .98em with `line-height:inherit`, stripped of tashkil, combining marks, the dagger alif and tatweel, it sits inside the body line box and the page holds one leading throughout. Nothing is lost, because the house entry form places a full transliteration immediately after every Arabic run and the transliteration is where the vowels are carried. Inline Arabic is for the term itself and short phrases; extended quotation belongs to the transliteration or to the display form below.

**Vocalisation is licensed at the display form only.** An Arabic span alone in its own paragraph renders as a centred display line with its own leading, where the extra height costs nothing because the line is its own. Authors reach this by putting `{{ar:...}}` on a paragraph by itself. Full tashkil is correct there and nowhere else in running prose.

The finding worth carrying: **a fix that removes a symptom by breaking the invariant the page exists to hold is not a fix**, and the tell is that it repairs one paragraph while leaving its neighbours untouched. The grid is checked by looking at a rendered page and not at the source, since neither collision nor leading break is visible in Markdown.

**Seal verification.** The verdict glyph is checked at 300 dpi before it is called tofu. At page resolution the three-dimensional angle and a stray arrow are indistinguishable, and a false tofu report costs a font hunt for a defect that was never there. Noto Sans Math is now named in the serif stack rather than reached by fontconfig fallback, which is what the III·E.1 pre-flight always required.

## III·E Quick Reference Card

Edition selector: `[EDITION:ENCYCLOPEDIA]` or front-matter `edition: encyclopedia`. Triggers: encyclopedia, encyclopaedia, reference work, alphabetic entries, dictionary pdf, lexicon volume. Routing sits between Scripture and Textbook; the seven document renderers never co-fire.
Page: royal octavo 494 by 736, margins 82 / 64 / 92 / 64, two columns at 168 pt with a 30 pt gutter, no column rule, no foot furniture.
Type: EB Garamond throughout, 9.5 pt body on 1.235, justified and hyphenated, 11 pt first-line indent, no bold anywhere in the body. Entry head 12.6 roman, sub 10 italic, running head 7.6 small caps, bibliography 8.1.
Accent: copper default, or crimson, slate, navy, forest, plum, via front-matter `accent`. It touches the letter bar, guide word, see-references, box top, and table bar only.
Guide word (III·E.2): the last entry to begin on or before the page, set automatically by `string-set`. Outer column on both verso and recto, folio inner. Never authored by hand. Letter openers and redirect stubs do not set it.
Entry (III·E.3): head, vocalisation, definition, body, see-also, signature, bibliography, seal. Redirect stubs are the ninth form and hold their filing position. `(q.v.)` is a pointer and a small-cap see-reference is a delegation; the two never collapse.
Letter opener (III·E.4): 46 pt capital, centred, 110 pt down, accent bar, 96 pt sink, breaks to a new page, no running head.
Grid (III·E.14): inline Arabic unvocalised at .98em inheriting the body line box; vocalisation licensed only at the display form, an Arabic span alone in its paragraph; paragraph-level leading overrides barred; seal glyph verified at 300 dpi; Noto Sans Math named in the serif stack.

Filing (III·E.7): article-stripped, NFKD-folded, hamza-dropped, lowercased string key. The engine warns on an order break and the warning is repaired at source.
Planning (III·E.6, III·E.8): apparatus at roughly a fifth of extent; tiers declared before drafting; mean length per tier fixed at volume one and audited each volume.
Source: front matter (`edition`, `title`, `volume`, `range`, `accent`), then `:::plate`, `:::front` with `:::section`, `:::corpus` with `:::letter` and `#` entries, `:::back` with `:::bmsection`, `:::idx`, and `:::artlist`.
Build: `python3 build_encyclopedia.py source.md /mnt/user-data/outputs/{slug}.pdf`; inspect the render; present the PDF; keep the Markdown as master.
Locks: no LaTeX, Unicode math only; no em-dashes; ﷺ as one glyph; no emoji; honorifics preserved through every gloss and index row; warrant tier on every claim-bearing box and table; Noto Sans Math provisioned or the ⟀ seal renders as tofu, which is a defect and not a render.
Module VIII (Author's Provenance and Method Disclosure) and the twelve-section paper template are both skipped for this edition, as for Textbook and Scripture.
...
**Gate 1 · Silent-Integration Scan (every paper, and every seated coordinate per II.C).** On a scholarly edition the scan also reads the author line and fails a byline that lacks the academic credential, per the credential law of III.B. At boundary two the scan additionally sweeps the card body for the auditor-erratum identifier prefix and for first-person revision constructions, "I was wrong," "my error," "correction to my," "previously I," "earlier I stated," each rewritten as native standing law or routed to the ledger.

 Search the artifact for: "v1," "v2," "v3," "vN.0," "with worked counter-example," "with discriminating," "audit," "revised," "this version," "the present paper specifies," "addressing," "in response to," "earlier," "previously," "fully executes" (often defensive), "now [restricted/specified/stated]" (often defensive). Most should be removed or rewritten as native discipline. Case-by-case judgment; some are content-legitimate in context.
...
**Mandatory after a sealed cycle.** Every paper that closes a FORGE cycle of `trisduction-audit-cycle` on SEALED-ROUND, HALT-STALE, or HALT-DIVERGENT leaves the session with its Blog edition beside the PDF and the master: rendered from the sealed master and never from a draft, at `[FORMAT:BLOG]`, carrying the same title, the byline with its credential per the credential law of III.B, the abstract, and a line stating the sealed version it renders; one version and one date across the three files. The Blog file adds no claim and owes no cycle; the Register-Transit Filter of Module IX runs on it at the Blog register on the way out. Step 10 of the Execution Pipeline presents it fourth.
...
# MODULE XI · THE POSITIONING DISCUSSION AND THE STANDING TABLE

## XI.0 · Operating Frame

A paper that draws on more than one literature owes its readers one statement of where it stands to each position it engaged, in words whose senses are fixed, and the statement is the paper's contribution read from outside: exactly the set of relations it carries and nothing wider. This module seats that statement as the close of Section 8 for the papers that can bear it. It fires automatically on every paper that clears the gate of XI.1 and is silent on every paper that does not; the inline tokens `[POSITION:ON]` and `[POSITION:OFF]` override the gate in either direction, explicit always winning, and an override that turns it on for an ineligible paper is logged as the author's ruling. The module authors no relation; it constrains the words a relation may be stated in and binds the statement to the body.

## XI.1 · The Eligibility Gate

Four conditions, conjunctive, ruled at Step 3b and logged in the working directory beside the V-FIO log, never published. One, register: the edition runs at `REGISTER:SECULAR`, which admits Apex Pristine, Journal, Math Journal, and Blog, and excludes Scripture, Textbook, Encyclopedia, and every artifact at the devotional register; a paper carrying theological content out of band remains eligible, since the table never carries that content. Two, field: the paper engages a citable scholarly literature, prior positions with authors and years, since a table of relations to positions needs positions. Three, mass: the paper carries at least one executed finding with receipts, a battery at E0, E1, or E2 with a seed and a digest, or a synthesis the literature does not hold, a road assessed under FORGET-01 and recorded in the Discussion's own contribution paragraph as Tier C; a paper whose whole contribution is accommodation and correction by reference, Tier A and Tier B, has nothing to position and the gate refuses it without prejudice. Four, the fence: the paper already carries its own statement of what it does not contribute, since the table's honesty rests on the same refusals; a paper without the fence is returned to draft it before the gate is re-run. The ruling is one line, ON or OFF with the failing condition named.

## XI.2 · The Relation Vocabulary

Eight words, defined once in the prose of the positioning run-in and used as defined in every row. **Additive**: the paper supplies a result or a test the position lacked and leaves it standing. **Replacing**: a metaphor or framing is retired and a measured object put in its place. **Subsuming**: the prior claim becomes a case or a consequence of the paper's object. **Corroborating**: independent agreement, which confers no warrant on either side and is counted as none, per W_social zero. **Contradicting**: a named thesis is denied on executed data and never on argument alone; the body must carry the sentence that denies it and the box or table that executes it. **Competing**: a different position on the same question, argued and not executed; the word a contradiction demotes to when its evidence is argument. **Scoping**: the prior claim is kept inside a stated boundary; reconciling, two positions kept at two rungs, is scoping's twin and may share the cell. **Kin**: a position the paper is continuous with, cited as such, with any open aperture named. **Superseding** is the ninth word and it is gated: permitted only where the paper retires a named theory's central claim on executed data at the same register as the theory, the register mismatch of gate twelve, ADEG, barring it outright; where the condition is unmet on every row the legend states that superseding is used nowhere and that the paper supersedes no theory, which is the honest line and the strongest one a room of specialists can be handed. A relation word applied outside its definition is an anchor inflation in the sealing direction and the Shield forbids it; the audit cycle prosecutes it under the claim of XI.4.

## XI.3 · Row Construction and the Fixed Columns

One row per prior position the body actually engages; a citation in the body is the license for a row and there is no other, so a position the body never discusses gets no row and the table is never padded to look comparative. Five columns in fixed order and no others: the prior position with author and year; what it holds, in one sentence in its own terms; what this paper does with it, in one to three sentences in the paper's terms; the relation, one word from XI.2, or two joined where a row is additive at one point and contradicting at another, each word carrying its own evidence; and the evidence, one word from a closed list, executed, cited, argued, documentary, open, with the executing box or table named where the word is executed. A row's evidence word never exceeds the tier the body carries for that position: a position the body engages by citation is cited, one engaged by a battery is executed, one engaged by argument is argued, and the row does not promote it. Every figure in a row is the body's figure at the body's rounding. Ten to thirty rows; a table under ten rows means the paper engaged too little to position and the gate should be re-read; a table over thirty means rows are padding.

## XI.4 · The Fidelity Law and the Audit Claim

The table states nothing the body does not. Every relation word corresponds to a body sentence that carries the relation at that grade; every evidence word matches the body's tier; every figure matches the body's rounding; and the census the prose states, so many replaced, contradicted, competed with, subsumed, added to, corroborated, scoped, kin, is recoverable by counting the rows. The table is registered as a pre-registered claim in the paper's audit cycle, in the form *the positioning tables: any row whose relation word is applied outside its defined sense, or whose evidence label exceeds the tier the body carries, or which states a result the body does not*, with that sentence as its falsifier, and the cycle prosecutes it in every round that reads the definitional or provenance register. A row the cycle finds inflated is repaired by demotion of the word, never by promotion of the body.

## XI.5 · Placement and the Prose

The positioning is the last run-in of Section 8, titled **How the paper stands to each prior position**, and the Discussion closes on its tables; the Conclusion follows and does not restate the table. The run-in is one paragraph of ten to sixteen sentences and does four things in order: it states why the statement is owed, the paper sitting across named literatures; it defines the relation words once, all eight and the gate on the ninth; it states the census by count with the positions named under each word; and it hands to the tables with the sentence that the paper's contribution is exactly the set of relations they state and nothing wider. No advocacy, no adjective on the paper's own work, no relation asserted in the prose that a row does not carry.

## XI.6 · Rendering

The table is spanning at the edition's table contract, `Table*:` in Journal and Math Journal, the box-and-table contract in Apex Pristine, a Markdown table in Blog. A spanning table that breaks across a page halts the WeasyPrint build, so the table is split into page-fitting parts numbered as consecutive tables, *Table N, part i of n, rows a to b*, the first caption carrying the pointer to the relation vocabulary and every later caption reading *same contract as Table N*. Eight rows of three-sentence cells fit a Letter page at the Journal contract; the split is tested by building, never assumed. The five columns keep their order in every part.

## XI.7 · The Handout Derivative

The same table travels as a conference handout when asked for, one or two landscape Letter sheets under the house typography, EB Garamond body, Liberation Sans furniture, copper accent, carrying in order: the venue line, the title and subtitle, the thesis in one block, the claims in six lines, the table, the legend defining the relation words in one paragraph, a box stating what the paper does not do and what it asks of the room, and a foot with the paper's length, criteria, batteries, and references counted. The handout is a public artifact that makes claims and it gets its own audit cycle before release, four claims and their falsifiers: number fidelity to the sealed paper at its printed rounding, relation words used as the legend defines them, evidence labels at the paper's tier, and no claim beyond the paper. The handout never precedes the sealed paper it is drawn from.

## XI.8 · Fences

No supersession by register mismatch: a floor that explains the inventory has not superseded a theory of the clause, and a row that says so is repaired by demotion. No contradiction on argument: the word demotes to competing. Convergence rows confer no warrant and the legend says so. The table carries no theological content and no honorific at any register, since it is a comparative instrument and not a devotional one, and its register is the paper's. No row without a body citation. The table is content and not disclosure, it does not replace Module VIII, and it is never placed in an appendix. The module does not fire on Scripture, Textbook, or Encyclopedia editions, on codex consolidations, or on any paper the gate refuses, and a refused paper closes its Discussion as §I.B Section 8 read before this module.

## XI.9 · Gate Integration and the Exemplar

The module runs at Step 3b of the Execution Pipeline, after composition and before titling, so that the Register-Transit Filter and the gate sequence audit the positioning as body text. Gate 2 of the pre-delivery sequence verifies, where the ruling is ON, that the run-in and the tables are present, that the census counts match the rows, and that the audit claim is registered; where the ruling is OFF, that the log carries the failing condition. The exemplar of record is *Unlettered Regenerative Floor* v16, Section 8, run-in and Tables 6 to 8, twenty-two rows in three parts, and its handout, both sealed by their own cycles on the day the module was written.

## XI Quick Reference Card

Trigger: automatic on every paper clearing the four-condition gate, secular register, citable field, mass in hand, the fence present; tokens `[POSITION:ON]` and `[POSITION:OFF]` override, explicit winning. Words: additive, replacing, subsuming, corroborating, contradicting on executed data only, competing on argument, scoping with reconciling, kin; superseding gated to a retired central claim on executed data at the same register, else barred and said so. Rows: one per engaged position, never padded, ten to thirty, five fixed columns, evidence in one word never above the body's tier, figures at the body's rounding. Fidelity: the table states nothing the body does not; registered as an audit claim with its falsifier; repaired by demotion only. Placement: the last run-in of Section 8, the Discussion closing on the tables. Rendering: spanning, split into page-fitting parts, tested by building. Handout: optional derivative under its own four-claim cycle, never before the sealed paper. Build: Step 3b; Gate 2 verifies.

---

# MODULE XII · THE RENDER-COMPLETENESS LAW

## XII.0 · Why this module exists

Added v1.8.0 after three builds of one paper shipped truncated while the
builder reported success. The failure is silent, it is not the author's,
and no rule in this file caught it, because every gate here checks the
source and none checked the artifact.

**The incident.** A 44,000-word Journal paper rendered to a PDF that ended
mid-sentence in Section IX. No Conclusion, no References, no positioning
tables. The builder printed `PDF: written`. The HTML intermediate was
complete and correct. WeasyPrint returned a document object without error
and reported a page count. Every visible signal said success and roughly a
third of the paper was gone. It shipped past two of this file's own gates
and was caught only because a reader asked why the page count had fallen.

## XII.1 · The mechanism, stated with its limits

WeasyPrint's `column-span: all` inside a multi-column `@page` context can
drop the remainder of the document. Four properties of the fault matter
more than the cause:

**It is silent.** No exception, no warning, no non-zero exit. The render
returns a valid PDF of the surviving pages.

**It migrates.** The stall lands at whichever spanning element first falls
badly, and that position moves with word count. The same source that
completes at 43,400 words truncates at 44,200. **A build that completes
today is not evidence the same source completes tomorrow.**

**It is version-dependent but not version-fixed.** WeasyPrint 65.1 carries
a longer document than 69.0 before failing. Pinning to 65.1 buys headroom
and does not remove the fault.

**CSS does not repair it.** `break-inside: avoid` was tested and does not
help. `break-before: page` was tested and does not help. Reducing rows per
table, trimming cell text, splitting one table into five, and separating
the spanning elements with prose were each tested and none helped. The
only configuration that is length-independent is **no spanning element at
all**, because a fault in column-span cannot fire where nothing spans.

## XII.2 · The law

**A render is verified by its end, never by its exit code.** No PDF leaves
a session, reaches a surface, or carries a seal until the built artifact
has been read back and confirmed to contain its own last line.

The probe is three checks on the built file and costs one tool call:

1 · **Terminal string.** Extract the text and confirm the document's final
sentinel is present. The Journal and Apex editions close on *End of
manuscript*; an edition without a sentinel gets one before it gets a seal.

2 · **Page count against expectation.** Record the page count of the last
good build. A count that falls while the source grows is a truncation
until proven otherwise, and that single comparison is what would have
caught all three incidents on the day.

3 · **Terminal sections present.** Confirm the Conclusion and the
References render, by name, in the extracted text. These are last in the
document and are therefore the first casualties of a stall.

A build failing any of the three is **not a deliverable**. It is not
presented, not filed, not uploaded, and not described as complete.

## XII.3 · The spanning-table disposition

§III·J's guidance stands unchanged for ordinary papers: prefer column
width, reserve spanning for tables too wide to read in one column. What
this module adds is a ceiling on that reservation.

**Where a document carries many spanning tables, or runs long, or is being
sealed, all tables render at column width.** The trade is explicit and it
is not close: a spanning table is wider and a truncated paper is missing.
Completeness outranks width at every register, and it outranks it
absolutely on a sealed artifact, because a build whose success flips with
every added paragraph cannot carry a seal.

Cramping is the cost and it is survivable. A five-column table in a single
journal column sets narrow and reads; a folded relation-and-evidence cell
recovers most of the width where the contract permits, per XI.3.

## XII.4 · The finding worth carrying

This file gates the source thoroughly and had no gate on the artifact, and
so a paper could pass every rule in it and ship as a fragment. The gate
that was missing is the cheap one, and it is the same shape as the
subtractive check the publication-automation skill learned at L9 after a
register was silently emptied: **a tool reporting success is not evidence
the work is intact.** Ask the artifact, not the exit code.

## XII Quick Reference Card

Fires on every PDF render, automatically, before presenting or filing.
Probe: terminal string, page count against the last good build, Conclusion
and References present by name. Any failure means not a deliverable.
Spanning tables are capped: long, table-dense, or sealed documents render
all tables at column width. WeasyPrint pinned at 65.1 for headroom, which
is mitigation and not a fix. **Completeness outranks width.**
