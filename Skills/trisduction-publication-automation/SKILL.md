...
# TRISDUCTION PUBLICATION AUTOMATION · The Unified Governance Layer · v2.6.0
...
1 · READ STATE. `GET /api/deposit/depositions/{id}`. Record `state`
and `submitted`. A record already carrying an open edit session
reads `inprogress` with `submitted: true`.

2 · UNLOCK. `POST /api/records/{id}/file-modification`. An empty
JSON body is accepted. It returns a request object that is created
and auto-accepted in one call, carrying `files.enabled: true` and
`is_locked: false`. This is the API equivalent of the front end's
