...
# TRISDUCTION PUBLICATION AUTOMATION · The Unified Governance Layer · v2.6.1
...
3 · DELETE. `DELETE /api/deposit/depositions/{id}/files/{file_id}`,
expect 204. Before step 2 this returns `403 Bucket is locked for
modifications`, and that 403 is the diagnostic that step 2 was
skipped rather than a verdict that the platform forbids the
operation.

4 · UPLOAD. `PUT` to the deposition's bucket link under the **final**
...
5 · METADATA. Revise in the same session, because the edit route is
...
6 · PUBLISH. `POST /api/deposit/depositions/{id}/actions/publish`.
...
7 · VERIFY SUBTRACTIVELY, and this step is not optional. Confirm the
