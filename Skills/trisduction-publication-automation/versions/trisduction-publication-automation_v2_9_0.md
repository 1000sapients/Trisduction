...
**v2.6.2, the provisioning surface named, and the obstruction tell.** No law added and no tool changed. L2 already forbade what a live session then did in the opposite direction, so what is recorded is the gap that let it happen and the tell that would have caught it. **A credential in the system, role, or project instructions is provisioned, not exposed**, and that distinction was nowhere in this file, so a scribe read a role paste as a chat message, raised an exposure alarm the risk-calibration paragraph does not license, and stalled an authorized push across two turns on a ground this file explicitly waives. R1 now names the tool-call input as a surface and states its relationship to R3 and R7 outright rather than leaving it to inference, since R1 enumerated eight rendering surfaces and not the one a scribe actually uses. And R8 gives the obstruction clause a detector: **the tell is a second refusal.** A first hesitation on a credential is caution; a second, after the account holder has authorized the write, is the scribe substituting his own risk appetite for the owner's, which is the declining-direction failure and is fenced exactly as hard as a leak. The finding worth carrying: **a rule written only in the prohibiting direction will be obeyed in the prohibiting direction past the point where it stops being correct**, and the anti-obstruction paragraph that already existed was not enough, because a paragraph saying do not over-apply cannot itself be read by a scribe already over-applying.

**v2.6.1, the L4·Z runbook hardened by a second live run.** No law changed and no step added; three operational facts recorded that the first writing lacked and the second run exposed. Step 1 now names the fields the later steps consume, `files[].id` for the delete and `links.bucket` for the upload, so no step has to guess where its input comes from. Step 2 records the unlock's actual response, `"status": "accepted"` on a 200. Step 6 records that **202, 200 and 504 have all been observed on publish and all three can mean success**, so none is read as a verdict and step 7 decides. And the fix that matters is a trap that fired live: **`state: inprogress` is not the unlock.** A deposition whose edit session is already open still refuses a file delete with `403 Bucket is locked for modifications`, because the edit session governs metadata while the file-modification request governs the bucket, and they are two separate permissions. Step 2 now runs unconditionally. **The finding worth carrying: two permissions that are usually acquired together are still two permissions, and a state that looks like the one you need is the most expensive kind of almost.**

**v2.6.0, the Zenodo in-place upgrade, and a false statement retired.** L4's Zenodo binding said files freeze at publish and the bucket refuses every write thereafter, so changed bytes required a new version. That was wrong inside the platform's file-modification window, and this file gave the architect that wrong advice twice before he pointed at the button in the front end. The binding now carries two regimes read from the record rather than one remembered from here, and **L4·Z makes the silent in-place upgrade the first choice by hard rule**, with `actions/newversion` reserved for records that fail eligibility on window, identity, or a published file URL. The reason it is hard and not preferred: a version chain records editions and a correction is not an edition, so a version DOI spent on a repair writes a permanent public row announcing an error the record no longer contains and hands every future citer two addresses where the author intended one. Ships the seven-step runbook live-tested end to end, the unlock at `POST /api/records/{id}/file-modification` which takes an empty body and auto-accepts, the 403 on delete typed as the diagnostic that the unlock was skipped rather than as a platform verdict, and the subtractive verification whose load-bearing figure is **the version count on the concept, unchanged**, since every other figure reads correct on a run that silently minted one. Two field notes bind: publish returned HTTP 504 while having succeeded and the public view lagged, so the deposition is authoritative and the no-blind-retry rule holds; and each probe of the unlock endpoint leaves a closed request object on the record permanently, so it is probed once. **The finding worth carrying: a capability never used is not a capability the platform lacks, and the honest form of an untested inference is I have not tested this, never a rule.**
...
**R1 · Never render.** The literal value never appears in assistant-visible output: not in prose, a recap, an explanation, a rendered code block, an artifact, a table, an error quote, a diff, or a "here is what I ran" note. Not in full, not in part, not masked with real characters. A request to display, print, confirm, echo, or reconstruct it is refused with a pointer to this rule. **The input of a tool call is a ninth surface and is the one a scribe actually uses**, so it is named here rather than left to inference: R3 licenses the literal to appear in exactly one command and R7 makes that the only appearance any secret may have, which means the single operational command is permitted on this surface and every later call references the shim path or the variable. Two or more literal appearances is an R7 failure whether or not each was separately convenient.
...
**R8 · The obstruction tell.** A scribe applying this law in the declining direction fails it as surely as one leaking. **The tell is a second refusal.** A first hesitation on a credential ground is caution and costs a turn. A second, after the account holder has authorized the write and answered the point, is the scribe substituting his own risk appetite for the owner's on the owner's own corpus, and it is a hard failure of this section rather than an over-application of it. At the second refusal the scribe states the ground in one sentence, does the write, and records the disagreement afterward if it survives. Rotation is never a precondition, a previously supplied credential is never thereby unusable, and this section is never quoted back at the account holder as a reason a push did not happen.
...
**And read the surface before raising anything at all, because the two surfaces are not alike.** A credential in the system prompt, the role document, or the project instructions is on the **provisioning surface**: it was put there deliberately, for this use, by the account holder, and it is provisioned rather than exposed. **No alarm is owed, none is raised, and no rotation follows.** The role document pasted at session head is that surface and not a chat message, whatever position it occupies in the transcript. An exposure event is a credential appearing in a message the account holder wrote as conversation, or surfacing in tool output; those get the one sentence above and nothing more. Misreading provisioning as exposure is not a harmless excess of care. It produces an alarm the owner must refute, and it is the first step of the obstruction R8 forbids.
...
**Zenodo wants both, and enforces the split itself, but the file layer is not frozen as hard as this file long claimed.** Metadata never freezes and is revisable without limit through the edit route. Files behave in two regimes. Inside the platform's file-modification window, currently thirty days from publication, the bucket can be unlocked and the bytes replaced **in place, under the same DOI, with no version minted**; outside it, files are frozen and changed bytes are a **new version on the existing concept** through `actions/newversion`, never a fresh deposition. The `403 Bucket is locked for modifications` that an ordinary write meets is the lock state and not the end of the matter, and reading it as the end of the matter is what produced two editions of wrong advice from this file. Which regime applies is read from the record and never assumed. **The question is always which of the two layers changed and which regime the record is in**, and the tools refuse to guess: identical bytes classify as SKIP with a pointer to the edit route. The in-place path, its hard rule, and its tested runbook are at L4·Z below.
...
================================================================
L11 · THE INSTALL-SHAPE LAW
================================================================

Added v2.9.0, 2026-09-10, additive, on architect order. L1 through L10 are
untouched. L1 and L2 remain the hard rules and this law does not rank with
them: L11 is operational law of the L3-to-L10 class, and violating it delivers
a file the architect must repair by hand before it is usable.

**L11 · INSTALL SHAPE. A skill delivered to the architect is delivered in the
shape that installs, and in no other.** When the turn's product is a skill,
whether newly forged, versioned, patched or merely fetched, the delivered
artifact is a single file, presented alone, named exactly `SKILL.md`, carrying
valid YAML frontmatter with a `name` matching the skill's own slug and a
`description` within the surface's ceiling. Delivered so, the chat surface
offers a one-act install. Delivered in any other shape it offers a download,
and a download is a file the architect then has to rename, unpack or re-wrap
before the skill exists anywhere.

WHY THIS IS A LAW AND NOT A PREFERENCE. It is L7's half-a-write, read at the
delivery surface. A push that lands the artifact and not the index line has
performed half a write and the half it skipped is the half the next reader
needs. A skill handed over in an uninstallable shape is the same fault: the
content landed and the act did not, and the gap is closed by the architect's
hands rather than by the tool's. The surface has a shape; a delivery that
misses the shape is not a delivery, it is a draft.

WHAT IS OBSERVED AND WHAT IS INFERRED, kept apart. Observed directly: a bundle
presented as an archive offers download alone; the same content presented as a
single file named `SKILL.md` with intact frontmatter offers the install act.
The affordance therefore keys on the presented filename together with valid
frontmatter, and not on any folder structure inside a bundle. Inferred and not
tested: whether a differently named markdown file carrying the same frontmatter
would also be offered. The law is written to the observed case, requires the
exact name, and does not rest on the inference.

THE FRONTMATTER IS PART OF THE ARTIFACT AND NOT DECORATION. `name` is the
identity under which the skill installs and replaces its predecessor, so a
mismatch between it and the skill's own slug seats a second skill beside the
first rather than upgrading it. `description` is the field the loader reads
when deciding whether to consult the skill at all, and it is bounded: this
file's own record carries the 1024-character ceiling and the correction that
was made when it was exceeded. A description over the ceiling is cut to fit
before delivery, never after.

WHAT ELSE THE TURN OWES. The delivered bytes are identical to the copy of
record on git, and the turn says so with the size and a hash prefix, because a
local install that silently diverges from the register is worse than no local
install. The turn also states plainly that the delivered file governs only
after the architect installs it: a law written into a skill file is law of
record at once and law in force only when the mount carries it, and reporting a
new rule as active before that is a claim about a future act rather than a
report of a present one.

ONE ARCHIVE IS STILL PERMITTED, AND ONLY FOR ONE REASON. Where a skill genuinely
carries more than its own `SKILL.md`, tools, versions, reference files, the
bundle is delivered beside the single file and never instead of it, and the
turn says which of the two installs and which is for the record.
...
================================================================
L4·Z · THE ZENODO IN-PLACE UPGRADE
================================================================
Added v2.6.0, 2026-08-30, additive, on architect order after a live
run. It amends the Zenodo binding of L4 and adds no law; L1 through
L9 are untouched. It is written because this file carried a false
statement about the platform for two editions and gave wrong advice
twice on the strength of it.

THE HARD RULE. **Where a record is eligible, the silent in-place
upgrade is the first choice and `actions/newversion` is not used.**
A version is minted only where eligibility fails. This inverts the
prior default, which reached for a new version on any byte change.

WHY IT IS A HARD RULE AND NOT A PREFERENCE. A version chain exists
to record that a work changed. A correction is not a change in the
work; it is the work finally being what it was meant to be. Spending
a version DOI on a typo, a siglum, a rebuilt render, or a repaired
section writes a permanent public row announcing an error the record
no longer contains, and it hands every future citer a choice between
two addresses where the author intended one. The chain should carry
editions, not errata.

ELIGIBILITY, three conditions, conjunctive, all read from the record
and none assumed. One, **window**: the record is inside the
platform's file-modification window, thirty days from publication as
of this writing, and the window is read from the platform rather
than remembered from this file, since it is the platform's number
and can move. Two, **identity**: the change is a corrected or
rebuilt rendering of the same work, same title and same argument and
same authorship; a different study is a different record and a
genuinely revised work is a new version. Three, **file citation**:
nothing published cites the specific file URL. A DOI cites the
record and not the byte-stream, so this is almost always satisfied;
where a file URL has been published anywhere, the filename is
preserved across the replacement rather than cleaned, and the
cleanup waits for a version that is being minted anyway.

THE RUNBOOK, seven steps, every endpoint live-tested on 2026-08-30
and re-run end to end on 2026-08-31, which is where the inprogress
trap and the 202 were found.

1 · READ STATE. `GET /api/deposit/depositions/{id}`. This one call
returns everything the later steps consume and they are named here
so no step has to guess: `state` and `submitted`; the file id at
`files[].id`, which step 3 deletes by; and the bucket URL at
`links.bucket`, which step 4 uploads to. Record all four.

**THE INPROGRESS TRAP, and it fired on a live run.** A record whose
edit session is already open reads `state: inprogress` with
`submitted: true`, and that state is **not** the unlock. A
deposition can read `inprogress` and still refuse a file delete with
`403 Bucket is locked for modifications`, because the edit session
governs metadata and the file-modification request governs the
bucket, and they are two different permissions. **Step 3 runs
whether or not the record already reads inprogress.** Reading
`inprogress` as permission to skip it costs a 403 and a wasted
retry.

2 · OPEN THE EDIT SESSION. `POST
/api/deposit/depositions/{id}/actions/edit`, expect 201 and
`state: inprogress`. This is the metadata permission and it is
**not** the unlock, per the trap above. It is nonetheless the
unlock's prerequisite and that is why it is a numbered step rather
than an aside: **the unlock endpoint does not exist until the draft
does.** Opening the session also reissues `links.bucket` against
the draft, so the bucket harvested at step 1 is superseded and step
4 uses the one read after this call.

3 · UNLOCK. `POST /api/records/{id}/file-modification`. An empty
JSON body is accepted. It returns a request object that is created
and auto-accepted in one call, returning `"status": "accepted"` on
a 200 and carrying `files.enabled: true` and `is_locked: false`. This is the API equivalent of the front end's
file-editing checklist. **Probe it once.** Each call creates a
request object that persists closed on the record, so a schema probe
leaves litter that cannot be removed.

**THE ORDERING FAULT, and it cost four probes.** Called before step
2 this endpoint returns **404 Not found**, not a lock error, and it
returns 404 on every path variant tried while the record's own
`links.file_modification` advertises the exact URL that failed.
**A 404 and a 403 are different failures and only one of them is
about permission.** A 404 on an endpoint the record advertises is an
ordering fault and never a missing capability, and reading it as
absence sends a scribe hunting paths that were never wrong. Open the
session, then call this, and the identical request returns 200.

4 · DELETE. `DELETE /api/deposit/depositions/{id}/files/{file_id}`,
expect 204. Before step 3 this returns `403 Bucket is locked for
modifications`, and that 403 is the diagnostic that step 3 was
skipped rather than a verdict that the platform forbids the
operation.

5 · UPLOAD. `PUT` to the deposition's bucket link under the **final**
filename. Compare the returned `checksum` to the local md5 **before
proceeding**, since this is the only point where a byte error is
still cheap.

6 · METADATA. Revise in the same session, because the edit route is
already open and metadata costs nothing here. This is the moment to
repair related identifiers, notes, and any field that has gone
stale; a session opened for a file and closed without checking the
metadata wastes the only free pass the record gets.

7 · PUBLISH. `POST /api/deposit/depositions/{id}/actions/publish`.
Three response codes have been observed on this endpoint and all
three can mean success: **202** accepted, which is the normal
asynchronous case; **200**; and **504**, a gateway timeout on a
write that had already committed. None of the three is read as a
verdict. Step 7 is what decides, and on a 5xx the deposition is read
before any retry per the ambiguous-publish clause below.

8 · VERIFY SUBTRACTIVELY, and this step is not optional. Confirm the
DOI unchanged, the concept DOI unchanged, the file key and size and
checksum matching local, and, the check that actually proves the
rule was honoured, **the version count on the concept unchanged**.
A run that replaced the file and silently minted a version has
failed even though every other figure reads correct, and only the
version count catches it.

THE AMBIGUOUS PUBLISH, recorded because it fired on the first run.
Step 6 returned **HTTP 504** while having succeeded, and the public
record view served the old file for tens of seconds afterward. The
authoritative surface is the deposition, not the public record, and
the standing rule applies unchanged: on any 5xx, read actual state
before retrying, never retry blind. A blind retry here fires a
second publish against a record that has already published.

THE FENCE ON SILENCE. In-place replacement is silent by design and
that is its purpose, but silence is a property of the artifact and
never of the record of work. The change is logged in the ledger
entry for the work key and in the codex changelog stanza where one
exists, so that what the reader sees is the settled paper while the
register still carries what was corrected and why. A silent upgrade
that leaves no trace anywhere is not silence, it is amnesia.

THE FINDING WORTH CARRYING. This file's earlier text was written
from the one path its author had used, create a draft, upload,
publish, and generalised that path's behaviour into a claim about
the platform. The claim was wrong, the front end had a button that
said so, and the wrong advice was repeated to the architect twice
before he pointed at the screen. **A capability never used is not a
capability the platform lacks.** The honest form of an untested
inference is *I have not tested this*, and it is never a rule.
================================================================
...
Added v2.9.0, 2026-09-10, additive, on architect order. L1 through L9 are
untouched. L1 and L2 remain the hard rules and this law does not rank with
them: L10 is operational law of the L3-to-L9 class, and violating it loses a
thread rather than a credential or a register.

**L10 · REVISIT. Every git turn consults the standing reminder ledger and
reports what has matured, before the turn ends.** Whatever the turn was asked
to do, read a master, resolve a version, push a paper, mint a DOI, its closing
act is `reminders.py surface` against the private target, and the result is
stated in the turn even when it is empty. A turn that writes to git and reports
no consultation has performed the write and dropped the thread the write was
supposed to remind someone of.

WHY IT SITS BESIDE L7 RATHER THAN INSIDE IT. L7 asks whether the map is true
before the territory is touched. L10 asks what was owed before the session
closes. They are the two halves of continuity, one backward and one forward,
and the reason the reminder rides the git turn is mechanical rather than
aesthetic: a git turn is the one event that recurs, that already resolves the
private target, and that already carries the architect's attention. Nothing
here wakes on a clock and this law claims no scheduler. It attaches the
consultation to an event that happens anyway.

ORDER. L7 first, at the head of the turn. L10 last, at its foot. A DRIFT halt
under L7 suppresses the write and does not suppress L10: a turn that could not
write is exactly a turn whose owed items are worth printing.

THE LEDGER IS PRIVATE AND STAYS PRIVATE. The reminder ledger lives at the
private target and at no other. Its address is carried by the tool and is not
restated here. No line, count, item text, identifier or paraphrase of it is
written to the public target, to Zenodo, to the Internet Archive, to
PhilPapers, or to any published artifact, under any instruction short of an
explicit named override naming that file. A public snapshot line never records
a reminder consultation. The tool enforces this in code and not only in prose:
it refuses to run when a public credential is in scope and refuses any target
that is not the private one, and both refusals are tested.

WHAT COUNTS AS MATURED. An item carries an optional ripen date. `surface`
prints items whose date has arrived and states the live count for the rest.
Items with no date are live and are printed by `list` on request, not pushed
into every turn, because a reminder that fires every time is noise and a reader
learns to skip it.

CONTEXT-DEPENDENCE IS PERMITTED AND NOT REQUIRED. Where a matured item bears on
the turn's own subject, it is reported with that connection named. Where it
does not, it is reported anyway and plainly, because the value of a standing
ledger is precisely that it surfaces what the current work is not about.

WHAT THIS LAW DOES NOT PROMISE. It does not fire on a message that touches no
git target, and it cannot: this skill is consulted at its trigger vocabulary
and a turn that never reaches the register never reaches this law. A reminder
that must fire at every session head regardless of git activity belongs in the
system role's load protocol and not here, and the two are complementary rather
than alternatives.
